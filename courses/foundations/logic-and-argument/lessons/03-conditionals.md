...
8. Magnus, P. D., Button, T., Thomas-Bolduc, A., Zach, R. et al., [*forall x: Calgary*](https://forallx.openlogicproject.org/) (CC BY 4.0), chapter 1 on arguments and chapter 2, "The scope of logic", which defines both validity and counterexamples: "An argument is valid if and only if the conclusion is a consequence of the premises. An argument is invalid if and only if it is not valid, i.e., it has a counterexample." Used for the validity test applied to each form.
