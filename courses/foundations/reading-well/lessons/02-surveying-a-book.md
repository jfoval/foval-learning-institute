...
objectives:
  ...
  - Decide on evidence from the book itself whether to read it, and say what the evidence was
quiz:
  ...
  - q: Adler's rule for a hard book on a first pass is to read it through without stopping to look up or ponder what you do not understand. Why is that not laziness?
    options:
      - Because the parts you grasp at a first reading are the parts that are worth having
      - Because most hard books are padded, so the hard parts of them are usually optional
      - Because difficulty is mostly vocabulary, and a dictionary would fix it later anyway
      - Because the first pass buys the shape, which is what the second pass then spends
    answer: 3
    explain: The rule trades local understanding for orientation, and it only makes sense because a second pass is coming, which is D. A abandons the hard parts rather than deferring them. B is a claim about books that neither Adler nor this course makes. C names one obstacle, and lesson 3 shows that missing background matters more.
  - q: You open a book whose contents page lists sixty short pieces by topic, with no index and no publisher's note. Which steps have returned nothing, and what should you do next?
    options:
      - Steps 1 and 5, so read the opening and closing paragraphs of three chapters
      - Steps 3 and 4, so dip in and read the last pages, which is what step 6 is
      - Step 2 only, so go back to the contents page and read it through more slowly
      - None of them, so the book has to be read straight through to be judged at all
    answer: 1
...
## What inspectional reading actually is

Adler splits the second level into two sublevels, which do different jobs and are easy to run together.[1]

**Systematic skimming**, which he also calls pre-reading, answers the question *is this book worth my week, and what is in it*. It has a time budget, and the budget is the point: "no more than a few minutes, at most an hour".[1]

**Superficial reading** is the rule for a hard book once you have decided to read it: "In tackling a difficult book for the first time, read it through without ever stopping to look up or ponder the things you do not understand right away."[1]
...
Here they are, in Adler's order.[1]

1. **Look at the title page and the preface.** Read both quickly. Note any subtitles or indications of scope, and place the book in a category in your mind.
2. **Study the table of contents.** Adler's instruction is to read it "as you would a road map before taking a trip".
3. **Check the index.** Estimate the range of topics covered and the kinds of book and author referred to. Look up the passages behind the terms that seem crucial.
4. **Read the publisher's blurb.** Adler's own view is that many blurbs say nothing, and that a blurb which says nothing has told you something about the book.
5. **Look at the chapters that seem pivotal to the argument**, and read their opening and closing summary statements.
6. **Thumb through the book**, "dipping in here and there, reading a paragraph or two, sometimes several pages in sequence, never more than that". And never fail to read the last two or three pages.

Read down that list and notice the shape of it. The steps run from the most compressed statement of a book to the least: a title, then an outline, then a word list, then a sales pitch, then some summary sentences, then actual prose. Each one tells you where to look in the next.
...
Surveying reads those encodings instead of the text. You are not skimming the argument. You are reading the compressed copies of it that the author and the publisher have already made, and which are sitting unread in every book on your shelf.
...
There is no preface in this edition, and the Introduction does the work of one. Its first sentence puts the author on a ship, and its last paragraph states the case: "I am fully convinced that species are not immutable", and "Natural Selection has been the main but not exclusive means of modification."[2]

Two minutes, and the book's claim is in your hands, in the author's words, with his own hedge attached.
...
One small warning if you are following along in the Gutenberg file: the heading is misprinted there as "DETEAILED CONTENTS", so a search for the correct spelling finds nothing. Which is itself a small lesson in reading an apparatus, since you are working with what the edition in front of you actually says.
...
Then the last page, which Adler says never to skip:
...
You have not touched the third and fourth. Those need analytical reading, which is lesson 5 onward. The survey tells you the shape of the argument, not whether it holds, and a reader who announces a verdict on the argument after an hour has skipped the level that would have earned it.
:::
...
You have surveyed, you have decided to read, and the book is hard. Here Adler's rule is the one most readers find counterintuitive: read it through the first time without stopping to look up or ponder what you do not understand.[1]
...
3. How is it built? Name the parts and what each one does.
4. Will you read it, and what is the evidence?
...
Lesson 1 asked you to choose a level. This lesson gives you the evidence to choose with: after a survey you know what the book is and can pick the level on purpose instead of guessing.
...
- **Adler and Van Doren, *How to Read a Book* (1972), chapter 4.** The six steps and both sublevels, in his own words and at more length. Your library will have it, and [Open Library](https://openlibrary.org/works/OL1892819W/How_to_Read_a_Book) lists the editions.
- **[Harvard Library, "Interrogating Texts: Six Reading Habits"](https://guides.library.harvard.edu/sixreadinghabits).** Its first habit is "preview", aimed at first-year undergraduates and free to read.
- **[Darwin, *On the Origin of Species*, first edition, at Project Gutenberg](https://www.gutenberg.org/cache/epub/1228/pg1228.txt).** Surveyed above. Worth opening even if you never read it, to see a Detailed Contents page, which almost no trade book published this year will give you.

## Sources

1. Mortimer J. Adler and Charles Van Doren, *How to Read a Book: The Classic Guide to Intelligent Reading*, revised edition, 1972. The two sublevels of inspectional reading, the six steps of systematic skimming with their wording, the time budget, the rule for superficial reading, the four questions a reader asks of any book, and the observations about blurbs and about the analytical table of contents. Consulted from a scanned copy; see `research/SOURCES.md` for the edition and for why this course does not link that scan to readers.
2. Charles Darwin, *On the Origin of Species by Means of Natural Selection*, first edition, John Murray, 1859, from [Project Gutenberg eBook 1228](https://www.gutenberg.org/cache/epub/1228/pg1228.txt), public domain. Title page and its two epigraphs, Introduction, Contents and Detailed Contents, index, chapter 9 on the imperfection of the geological record, and the closing paragraph. All quotations checked against that text.
3. Francis Bacon, *Essays or Counsels, Civil and Moral*, 1625, from [Project Gutenberg](https://www.gutenberg.org/cache/epub/575/pg575.txt), public domain. Surveyed above for its contents page, and for the absence of an index and of a publisher's note.
4. [Harvard Library, "Interrogating Texts: Six Reading Habits to Develop in Your First Year"](https://guides.library.harvard.edu/sixreadinghabits). Habit 1, "preview", as the modern short form of systematic skimming.
