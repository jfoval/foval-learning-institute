...
Thirteen lessons have given you a picture of where a file is, how to stand somewhere in a terminal, what happens between an address and a page, and what you are actually trusting when you install something. This project is the first time you use all of it at once, on your own machine, about your own life.
...
The file being plain text is not an arbitrary rule. Producing it is lesson 3's exercise done for real, and when you finish you are holding a document you made with the skills the course taught, on a machine you understand better than you did. Nobody can write it for you, because every section is about your machine and your accounts.

Budget about fourteen hours over two or three weeks. Some of that is waiting, for a restore or for a provider's documentation. Section 6 isn't waiting: it is the longest piece of real work here and it is the one that changes what happens to you.

## Before you start

Open a plain text file and keep it open. Call it `audit.md` or `audit.txt`, and put it somewhere you can reach from the terminal in one `cd`. You will be pasting into it all the way through, and the transcripts matter as much as the prose.
...
Go back to the security plan you built in lesson 1, from the five questions: what you have that's worth protecting, who might want it, how likely they are to try, how bad it would be if they succeeded, and how much trouble you are willing to go to.

Write it again now, and then do the thing lesson 1 does to it at the end.

**Add the sixth question.** "What can my machine and my connection be used for, by someone who does not care who I am?" That question is outside the five, which is lesson 1's point: the first five all assume an attacker who is after contents, and the sixth is the one that catches the attacker who only wants your connection. Answer it in writing.

**Your revised plan has to contain at least one row you couldn't have written in week one.** That row is the point of the section. It may well come out of the sixth question, which is where it comes from for most people, and it may come from anywhere in the course: lesson 3's five causes of a missing file, lesson 7's parties in the path, lesson 12's copy that is not a backup. Say which lesson put it there and what the row says now.

If you genuinely cannot find such a row, say that instead, and say why. An honest "nothing moved, and here is what I already knew" is worth more than a row invented to fill the space.
...
So when you write the revised plan, do not check it against a list of precautions. Check it against the pictures. A plan whose every row assumes somebody who wants your money in particular is the big fish picture, and the sixth question above is the answer to it.
:::

## 2. The map

Design a directory, build it from the terminal, and say why it is shaped the way it is.
...
Lesson 2's fourth objective was predicting which addresses stop working after a rename, and the reason this is worth doing at full size is that the three cases do not behave the same way, which a small tree never shows you.

## 3. The criterion

Create `hello.py`, run it, and break it three ways on purpose.

This is lesson 5's exercise, and the name is deliberate: it's the criterion for whether the terminal half of this course worked. Somebody who can do this can diagnose a machine. Somebody who cannot is still guessing.

The three breakages are the three categories:

- **Where you are standing.** The file exists, the program exists, and you are somewhere else.
...
What you are testing is not memory of lesson 5. It is whether you can say, in advance, how far the machine got in each case, because how far it got is what decides how much it has to tell you. A failure with no position to report has nothing to say beyond what it couldn't find.
:::

## 4. The pipeline

Answer one question about your own files that you could not have answered by clicking.

The question has to be real. "How many files are in this folder" is answerable by clicking and does not count. The ones that work have a condition in them:
...
In the file: the finished pipeline, its output, and **one sentence per stage saying what that stage contributed**. If you cannot say what a stage contributed, it isn't earning its place and the pipeline is one command shorter than you thought.
...
:::callout What you should expect depends on which machine you are on
On macOS and Linux, a script a text editor made is not marked as runnable, and you will get the fourth category: the file was found and the system declined to run it.
...
Either way, run `ls -l` and read the permissions, and write down what you see. The reading is the skill, and it is the same reading on a server you meet later.
:::
...
**Before you read your output, predict the party list.** Write down, from lesson 7's figure, every party you think can tell you visited that site, and the one you think cannot. Then read your own output and see how many of them you can find evidence of with your own eyes.
...
- **The one who cannot**, with the reason. The reason is not encryption, and this is worth getting right because it's the answer almost everybody reaches for. The networks in the middle see an address rather than a name, and one address at a delivery network commonly answers for thousands of different sites, so they can say your traffic went to that machine and cannot say which of those thousands you were reading. Encryption does not remove a party from the path at all. It changes what they can read, which is a different claim and is lesson 8's.
...
One thing you will see and should say something about: the rows of asterisks. They mean those machines chose not to announce themselves, which many are configured not to do, and the page loading is the evidence that the path works.
...
- **Which rung of lesson 10's ladder you reached**, and which attack that rung stops. Not which one it helps with. Which one it stops, and which one it does not. An app code stops a breached password being replayed by machine and does not stop a convincing page relaying it in real time.
...
- **What you could not fix, and why.** A service with no second factor. An employer's policy you cannot override. A recovery phone number you no longer control. This row is required, and a project with nothing in it is a project that stopped looking.
...
Not a test file you created for the purpose. A file you would actually miss, restored from the place you would actually go.
...
3. Write down which of lesson 12's five parts you are least sure of: the schedule, the selection, the destination, the format, or the credentials.
...
1. The plan, revised, with the sixth question answered and at least one row you could not have written in week one.
...
6. The hardening: your first ordering, five accounts, five rungs, five recovery paths, and what you could not fix.
...
| The transcripts are real | Output is pasted from your own machine, including the parts that did not work |
...
**The last two rows are this course's own standards, turned around and applied to you.** Every lesson here dates its figures and says where its evidence runs out, and two of them stop to explain why they will not print a number the research could not stand behind. Asking you to do the same isn't a formality. An audit that hides what it could not fix is exactly the kind of document lesson 8 taught you to sort into measurable, unmeasurable and false.

## What this is not

It is not a checklist you tick. Five of the seven sections ask you to write a prediction down before you act, and the prediction is where the learning is. Skipping to the doing produces a file that looks finished and taught you nothing.

It is not a security score. There's no number at the end and no threshold you pass. Lesson 1 was honest that understanding a computer does not by itself make a person safer, and a project that ended in a score would be quietly claiming the opposite.

And it is not finished when the file is. The row you could not fix in section 6 is a thing to go and fix, and the provider whose retention window surprised you in section 7 is a decision you now get to make.
