...
quiz:
  - q: >-
      A parish clerk asks for the rule on how much notice a council must give of an extraordinary
      meeting, and gets a fluent answer with a section number. She checks: the Act exists, the
      section exists, and it is about something else. Which failure is this, and why is it the harder
      one?
    options:
      - An invented source, and it is harder because a made-up Act is difficult to search for
      - A real source cited for something it does not say, and it is harder because every check short of reading the section passes
      - A misreading, and it is harder because the system will repeat it when asked again
      - A retrieval error, and it is harder because the fault lies with the search rather than the system
    answer: 1
    explain: >-
      Everything that is cheap to verify came back clean, and only opening the section catches it,
      which is why this kind survives a careful reader. The invented-source answer describes the
      easier failure, since a source that does not exist fails at the first search. The misreading
      answer imports a step that did not happen. And the retrieval answer assumes a search took
      place; nothing here says one did.
  - q: >-
    ...
    options:
      ...
      - They are the same account in different words, since incentives are part of the design
      - "They are complementary and imply different things: one a floor, the other a lever nobody has pulled"
      - The incentive account is newer, so it supersedes the design account for anything current
    answer: 2
    explain: >-
  ...
  - q: >-
    ...
    explain: >-
      ...
      cannot-be-measured answer is what the researchers had to work around rather than a fact: they
      built a preregistered evaluation and published the rates.
  - q: >-
  ...
  - q: >-
    ...
    options:
      - Nothing has gone wrong; hedged answers are produced when the system is less certain, so they carry information
      - Hedging is a property of the text in the same way fluency is, so the reader has swapped one bad signal for another
      - Hedged answers are usually wrong, so the reader has inverted a rule that was right to begin with
      - The rule holds for facts and fails for opinions, so it needs a qualifier rather than abandoning
    answer: 1
    explain: >-
      Both fluency and hedging are features of produced text rather than reports on anything, so
      reading either as a confidence meter is the same mistake with the sign flipped. The
  ...
  - q: >-
    ...
    answer: 2
    explain: >-
      Those two questions map onto the two failures exactly, and the second is the one most people
      skip because the first has just passed. Recency and reputation are real quality signals about
...
Ask why these systems produce confident false answers and you will be given one of two explanations. They're usually offered as rivals. They aren't rivals, they're compatible, and the reason to hold both is that they disagree about whether the problem can be fixed.

**The first is structural.** NIST's profile puts it as the design working rather than failing: confabulations "are a natural result of the way generative models are designed: they generate outputs that approximate the statistical distribution of their training data".[1] Lesson 2 built its whole argument on that sentence. On this account there's a floor, because producing text that fits the shape of writing will sometimes produce text that fits the shape of true writing without being true.
...
On the incentive account, something specific would have to have changed about how the model was trained or scored, in a direction that makes abstaining worth something. That's a claim you could check: what did they change, and what does the model now do when it doesn't know?
...
**An invented source.** An author who may exist, a journal that does exist, a title in the right style for the field, a year, a volume. Lesson 2 explained why it looks so right: the shape of a citation is in the training material tens of millions of times, so producing a well-formed citation is exactly what a system producing well-shaped text will do.

This one's annoying and it isn't the dangerous one, because it fails the first check you run. Search for it and it isn't there.

**A real source cited for something it does not say.** The paper exists. The section exists. You look it up, confirm it's real, and stop, because everything you thought to check has passed.

This is the one that ends up in published work, and the evaluation in the next section defines the two separately for exactly that reason.
...
**So the check is two questions rather than one.** Does this source exist? And does it say what it is being cited as saying? Almost everybody asks the first and stops.

## What happens when you connect it to real documents

Here is the objection every reader has by now. *Mine searches real documents before it answers. Surely that fixes it.*

It's been measured, and the measurement is worth more than any argument either way.

In 2024 a team from Stanford and Yale ran the first preregistered evaluation of commercial legal research tools: products built for professionals, connected to real case law, sold on the claim that the problem had been dealt with. The providers had said their methods were "eliminating" or "avoid[ing]" hallucinations, or guaranteeing "hallucination-free" legal citations.[3]
...
Both halves of that sentence matter, and most writing carries only one.
...
**And between 17% and 33% isn't "solved".** On tools sold to lawyers, tested in 2024, one answer in four or five.
...
And note how the study had to define its terms to get a number at all: a hallucination is "a response that contains either incorrect information or a false assertion that a source supports a proposition".[3] Two failures, separated, which is the distinction the previous section drew and is why it is worth drawing.
...
Don't carry the numbers. 17% to 33% is a measurement of three named products on a particular task in a particular year, and your product isn't one of them. If somebody quotes those figures at you about a different system, they've done the thing this course keeps warning about.
...
NIST says plainly where the damage comes from: risks arise "when users believe false content -- often due to the confident nature of the response -- leading users to act upon or promote the false information".[1]

The fluency is a property of the text. It isn't a measurement of anything, and it isn't a report on the system's state. Digital Literacy made the same point about a padlock: a signal that costs nothing to produce cannot tell you what you wanted to know, and a well-shaped confident sentence costs a system that produces well-shaped text exactly nothing.
...
So the honest position is neither of the two you'll hear. "It has no idea whether it's right" is wrong, and so is "just ask it how confident it is". **Self-assessment is real, better on familiar closed-form questions than on unfamiliar open ones, and not a substitute for checking.** Lesson 8 puts it to work as one step among four, which is the right weight to give it.
...
**"It only happens on obscure questions."** NIST's own sentence says the dynamic is particularly relevant on open-ended prompts, long-form responses, and specialist domains.[1] That isn't obscurity, it's depth, and it describes the work you most want help with.

**"If it cites a source, the source is real."** Sometimes, and the important half is the other question: if the source is real, does it say what it is cited as saying? The legal evaluation had to define a hallucination as either kind, precisely because both happen.[3]

**"A confident tone means firm ground."** Fluency is a feature of text. So is hedging. Reading either as a confidence meter is the same mistake with the sign flipped.

**"Telling it not to make things up helps."** The instruction is in the request, and on both accounts above the problem is upstream of the request. Whether it does anything at all on your product is a measurement this course doesn't have, so the honest answer is that it isn't the lever, and you shouldn't buy safety with it.

## Practice

:::exercise Three sources, three kinds
Take 30 minutes. Do this in a subject where you can actually check, which usually means your own field.

1. Ask for three references on something specific and slightly obscure in your subject.

2. **Predict before checking.** How many will be real and support the claim? How many will be real and not support it? How many will not exist? Write the three numbers down.

3. Check all three properly. Not by asking the system. By finding the source.

4. Sort them into the three kinds, and compare with your prediction.

5. The half most people skip: for the ones that exist, **read enough of the source to know whether it supports the claim it was attached to**. This is slower and it's the whole point of the exercise.

Keep the result. Lesson 8 builds a checking procedure and this is the evidence for why its first step is what it is.
:::

:::exercise Find the harder failure on purpose
Take 20 minutes, and treat it as a hunt rather than a test.

Ask for a summary of a document, a paper or a report you know very well. Something you could be examined on.

Then read the summary looking for one specific thing: **a sentence that is true, attached to a citation or a passage that does not support it.** Not a false claim, which you'll spot easily. A true claim with the wrong support under it.

If you find one, write down what made it survive your first reading. If you don't, write down how long you spent looking, because that number is the real cost of this kind of checking, and lesson 8 asks you to budget for it.
:::

## Connections

**Back.** Lesson 2 gave you the mechanism this whole lesson rests on, and the explanation of why an invented citation looks right. Lesson 5 met confabulated logic from the other side, as steps you can check. Lesson 6 told you which tasks put you here in the first place, and the answer, the ones whose answers have to match something outside the request, is exactly where this lesson's failures live.
...
- **[NIST AI 600-1](https://doi.org/10.6028/NIST.AI.600-1)**, section 2.2. Two pages. The best short account anywhere, and it's from a standards body rather than from somebody selling or attacking a product.
- **Kalai, Nachum, Vempala and Zhang, ["Why Language Models Hallucinate"](https://arxiv.org/abs/2509.04664)** (2025). This course has read the abstract and not the paper. The exam analogy in it is the clearest way anybody has put the incentive argument.
- **Magesh and colleagues, ["Hallucination-Free? Assessing the Reliability of Leading AI Legal Research Tools"](https://doi.org/10.1111/jels.12413)** (2025). Open access. Read section 4 for the typology, which is the most transferable thing in it: their way of separating a false claim from a misused source works in any field.
