...
## Lesson 9, "How the text reached us, and how reliable it is" — Stage 4 round 1, 2026-09-08

Five fresh-context subagents: depth, fact-check, neutrality, voice and media, pedagogy. All five
returned. Every finding below was applied in the same session unless marked otherwise. The lesson
was not held for review; it is fixed and stays live-eligible.

### BLOCKING, applied

- **The timeline chart misdrew its own data.** Three reviewers caught it independently. The axis is
  linear at 0.2 px per year, and on that scale Isaiah's composition bar ran 700 to 125 BC against a
  label reading "8th to 6th c. BC", and Paul's ran AD 50 to 200 against a label reading "letters, AD
  50s". The bars had been stretched to touch their manuscript dots, which inverts the chart's whole
  argument by shrinking the gap it exists to show. The chart was redrawn from scratch at a larger
  scale: correct bar geometry, and every label at 15px or above, which also cleared the validator's
  standing 4.6 warning on this lesson.
- **The 400,000 figure was attributed to Ehrman as well as Wallace, and to sources that do not
  carry it.** Neither footnoted page gives a total. The figure is Wallace's, from his 2013 post "The
  Number of Textual Variants: An Evangelical Miscalculation", now fetched, quoted and cited as note
  29. Ehrman's own published estimate is a range, 200,000 to 400,000. Body and quiz both rewritten.
- **Ehrman's concession was quoted with his own qualifier removed.** The blog sentence ends "(at
  least the variations that we know about)". Trimming a sceptic's hedge off the concession the
  lesson leans hardest on is exactly what 3.2 and 3.7 forbid. The parenthesis is restored and the
  predict block now says the parenthesis is his and belongs in the quotation.
- **Ehrman's examples were given in their weakest form.** The lesson had "whether Luke's voice at
  the baptism says 'today I have begotten you'" where Ehrman's claim is that the variant bears on
  whether Luke's Jesus is God's Son eternally or becomes so at his baptism, and "whether Luke's
  Gospel includes the words about the body given for you" where his claim is that on the shorter
  reading Luke has no doctrine of atonement at the Last Supper at all. Both are now stated at full
  strength with verse references, which is what 3.7 requires and what the lesson's own credibility
  move depends on.
- **"No article of the creeds depends on a disputed reading" was filed under history.** It is a
  judgement about what the readings amount to, not a measurement. It now has its own paragraph,
  labelled as a judgement, with Ehrman's circularity caveat and the three variants critics actually
  point to (1 Timothy 3:16, John 1:18, Hebrews 2:9) named so a reader can look them up, followed by
  Wallace's reply.
- **The quiz had zero application items and was guessable by shape.** The correct answer was the
  longest option in five of six and carried a hedge in five of six; all six correct answers restated
  a sentence from the body, and one restated a checkpoint's hidden answer. The Old Testament half of
  the lesson, about 40 percent of the body, was tested by nothing. The quiz is now eight items, four
  of them applying the method to cases the lesson does not work (an unseen Old Testament footnote, a
  harder-reading decision, a classical text with three manuscripts, and Luke 22:43–44), with the two
  strongest original items kept.
- **Sourcing that repeats a defect this course has escalated three times.** Tov was characterised
  and paraphrased, including a claim about the shape of his chapters, though the research file never
  records the book being opened; the same for Metzger on the creeds. Both are now attributed to what
  was actually read, and note 2 says plainly that the book has not been opened here.

### Facts corrected

- "Nineteen centuries of copying by hand" is wrong: hand-copying ended with print. Now fourteen
  centuries of copying and four of printing.
- Vaticanus is not a complete Greek Bible (it lacks the Pastorals, Philemon and Revelation, and
  breaks off at Hebrews 9:14). The quiz option now reads "the two oldest Greek manuscripts of the
  Gospels", which is also Metzger's own wording.
- Sinaiticus does not leave a notable blank after Mark 16:8; the anomalous blank column is
  Vaticanus's. As written the quiz explanation handed a KJV-only reader a real error to catch.
- The Aleppo Codex's losses are not a settled 1947 fire. Roughly 40 percent of the leaves are
  missing, no evidence of fire has been found, and when they disappeared is argued. Said that way now.
- "Every modern Old Testament translation rests on the Leningrad Codex" is overstated twice over: a
  translation is made from BHS or BHQ, editions based on Leningrad, and the Hebrew University Bible
  Project and Jerusalem Crown use Aleppo. Body and figure caption both corrected.
- P52 is asserted as "the oldest" and then undercut two sentences later by Nongbri's redating. Now
  "the one usually called the oldest".
- The 5,800 manuscript count has no reachable INTF source; published totals range from about 5,600.
  Given as a range, with the claim resting on Wallace's quoted words and the register linked.
- Mark 1:41 is not in the blog post it was cited to; it is from *Misquoting Jesus* chapter 5, now
  note 30.
- The Isaiah Scroll is seventeen sheets of prepared skin, about seven metres, not "leather".
- "No scholar of any stripe dates them more than about seventy years after the crucifixion" is an
  absolute that fails on contact with mainstream dates for John. Replaced with the actual ranges.
- Three quotations were tidied silently (a dropped "Yet, even", an inserted "and", a truncated "of
  the evangelicals"). All restored.
- Ketef Hinnom now cites Barkay, Lundberg, Vaughn and Zuckerman, *BASOR* 334 (2004), rather than
  "the standard excavation reports", which named nothing.

### Neutrality (standpoint: christian, standards 3.7)

Perspectives checked: critical text criticism (Ehrman, Parker, Epp, the Münster initial-text
position), thoroughgoing eclecticism (Kilpatrick, Elliott), Byzantine priority (Robinson, Pierpont,
Hodges), conservative evangelical (Wallace, Hurtado, Kruger), mainline (Metzger), Hebrew Bible
scholarship (Tov, Ulrich), Catholic, Orthodox and Jewish.

- Metzger was called "the field's most respected conservative", which he was not; he was a mainline
  Presbyterian who did not hold inerrancy, and the label made a centre-to-centre agreement look like
  a convergence across the ideological range. Relabelled throughout, along with "the critic" for
  Ehrman, who describes himself as a historian and an agnostic.
- Reasoned eclecticism was taught as simply how the field works. Its two live rivals are now named
  in the body, with Byzantine priority given in the form its serious advocates hold rather than the
  vote-counting version that appears as a quiz distractor, and the course's own position stated.
- The textual plurality of the Hebrew Bible was understated to the point of changing what it shows.
  "Some copies of Jeremiah and Samuel match the Septuagint" is now the actual case: a Jeremiah about
  an eighth shorter with the oracles in a different place, which is two editions of a book, plus the
  1 Samuel 17 to 18 effect a reader can see in their own Bible.
- The Septuagint was treated as a witness without saying that this is a Protestant and Catholic
  decision. The Orthodox position is now stated in its own terms.
- A straw sceptic ("a sceptic who says only the third is hiding more") closed the section that had
  just quoted Ehrman saying all three things. Rewritten symmetrically.
- The predict block framed Ehrman's concession as a reveal. It now asks for both halves of his
  position, which is what the standard means by engaging an objection at full strength.
- Also fixed: "late and thin" of the Textus Receptus, "bad arguments about which Bible to trust",
  "not a fourth-century invention" where the hypothesis was a recension, and the "hasn't read them"
  jab, which cut both ways but was still a jab at persons.

### Pedagogy

- The lesson worked three passages and never gave the learner a procedure for a fourth. A six-question
  procedure now sits before the worked examples, followed by the vocabulary of the notes themselves
  (what "some manuscripts" means against "the earliest manuscripts", single brackets, double
  brackets, a bare footnote) and a route to the NET's translators' notes, which the course's own
  research file has recorded as the best free tool for this and which the lesson had never mentioned.
- The harder-reading criterion, the most transferable idea in the field, appeared once, in a
  subordinate clause, inside a checkpoint body the site hides behind a button. Promoted into the
  mechanism section and now tested.
- Objectives rewritten: the first bundled four assessables behind one verb, and the fourth asked for
  something no multiple-choice item could test. There are now five, including one that names the
  transfer skill the new exercise and quiz item test.
- Exercise item 3 asked the reader to look and remember, which is not practice; it now asks for a
  count and two sentences. A new item 4 runs the method on a passage the lesson does not work.
- A misconception ("Sinaiticus and Vaticanus are the Bible") told the reader that sometimes other
  witnesses outweigh the codices and never showed a case. It now points at one the reader has read.

### Voice and media

- Two banned sentence shapes at the top: the "by the end you'll be able to" promise, deleted, and
  four stacked rhetorical questions, cut to one. Also "Let's clear away" and a sentence whose whole
  job was to announce the next sentence.
- Seven paragraph-closing one-line morals, four of them the same paired-symmetry move. Two kept, the
  rest cut or flattened.
- The 78-word summary sentence, four semicolon-joined clauses hanging off one verb, split into four.
- The body had two links in it. Nine primary sources are free online and were verified: Wallace's
  interview, Ehrman's post, all three of Metzger's entries at bible-researcher.com, the Münster
  register, the IAA archive, Vaticanus at the Vatican Library. All are now linked at the point of
  use, which is what 4.5 asks for.
- Two images added, both public domain and both verified: the end of Mark in Vaticanus, which is the
  one picture the lesson kept asking the reader to imagine, and Codex Montfortianus, the manuscript
  produced when Erasmus was challenged over the Comma.
- The Israel Museum's Digital Dead Sea Scrolls host did not resolve; the exercise now sends readers
  to the IAA's Leon Levy library, which does and which is already a source.
- Section heading "a thousand years in one afternoon" promised something the section does not do.
  Now "a thousand-year gap, closed in 1947".

### Unverified, and said so rather than passed

- Metzger and Ehrman, *The Text of the New Testament*, 4th ed.: not available online, no page
  numbers anywhere, not one claim from it verified. The lesson no longer rests anything load-bearing
  on it and note 2 says so.
- The NA28 and UBS5 double brackets at Mark 16:9–20 and John 7:53–8:11: near-certain, universally
  reported, neither edition openable.
- The P75-and-Vaticanus finding: standard, primary study not fetched.
- The blank space after Mark 16:8 in Vaticanus is standard scholarship but was not verified from the
  image at its available resolution. The caption states it; if a higher-resolution folio from
  digi.vatlib.it can be sourced, prefer it.

### Deliberately not fixed

- The reviewer suggested splitting the lesson at the Old Testament / New Testament seam. Not done.
  The two halves serve one argument and the quiz now covers both; splitting would make lesson 9 the
  only lesson in the course whose halves cannot be read without each other.
- The checkpoint that poses a weighing problem before the first fully worked weighing stays where it
  is. The mechanism is stated immediately above it and the block reveals its own answer, so it is
  doing desirable-difficulty work rather than testing what has not been taught.

## Lesson 10, "What we believe about this book, and what others make of it" — Stage 4 round 1, 2026-09-08

Five fresh-context subagents. All five returned; everything below was applied in the same session.
This lesson is the doctrine-of-Scripture lesson and the most sensitive in the course, so the
neutrality brief asked for fairness in two directions at once: to the critical account from outside
the faith, and to the Catholic, Orthodox, mainline and inerrantist positions inside it.

### BLOCKING, applied

- **The Chicago Statement was quoted only in its permissive half, for the third time in this course.**
  SOURCES.md carries a standing instruction, written at lesson 3's Stage 4 and repeated at lesson
  6's, that any lesson citing Chicago for genre latitude must cite its limits alongside it or it
  over-reads the statement in the direction its own signatories reject. This lesson leaned harder on
  Article XIII than any other and cited neither limit. Both are now in the body: Article XII's
  second denial, and the 1982 Chicago Statement on Biblical Hermeneutics, which values genre
  criticism and then denies that "generic categories which negate historicity may rightly be
  imposed on biblical narratives which present themselves as factual".
  **Correction to the research file:** that clause is Article XIII of the 1982 statement, not
  Article XXII as SOURCES.md records it, and Article XIV is the companion. Fetched and quoted from
  the full text; SOURCES.md corrected.
- **Article XIX was quoted with its second half removed.** The lesson used "we deny that such
  confession is necessary for salvation" to support the claim that a Christian who takes the
  source-critical side has not left the faith. The article goes on, in the same breath, to deny
  "that inerrancy can be rejected without grave consequences, both to the individual and to the
  Church". Cutting that is the same failure the course keeps having to fix, and here it cut in the
  inerrantists' disfavour. Both halves now stand.
- **The course's own central argument faced no objection.** Every other position in the lesson gets
  a reply printed against it; the argument from Jesus' view of Scripture got "that's the whole
  argument, and we find it strong". Standards 3.7 does not exempt the house position. The two
  standard objections, circularity and accommodation, are now stated in their holders' terms with
  the replies, and the second is tied to the Pentateuch section, where this course explicitly does
  not treat "Jesus said the book of Moses" as settling authorship.
- **The critical view was represented by its manners rather than its conclusions.** Every quotation
  chosen from the critical side was a disclaimer or a limitation. Not one substantive critical
  result appeared. The results are now stated plainly, in the form a critical scholar would state
  them, along with the fact that some scholars hold they count decisively against any doctrine that
  calls this book God's Word, and a pointer to Ehrman at full length rather than in summary.
- **Books characterised that nobody opened.** Wenham, on whom the course's own faith argument rests,
  plus Rogers and McKim, Enns, and a claim about ancient Near Eastern parallels attributed to three
  unopened conservative works. All now say in the body that the description is second-hand, and the
  unsourced clause is deleted. The lesson no longer reports what Enns's postscript says.
- **The quiz had no application items.** All six wrapped a new speaker around a case the lesson had
  already worked, and four were the four bullets of the misconception list restated. Five items
  replaced; four of the six now work cases the lesson does not, including one that tests what a
  hypothetical new manuscript would and would not change, which is the only item that tests the
  "what would move it" half of the sort.

### Facts corrected

- The lesson said a stele moved "David existed" from argued to established. SOURCES.md records that
  exact overstatement and its correction, and the lesson's own archaeology section and quiz get it
  right; only this sentence was wrong. What the stele establishes is a dynasty named after a David.
- Miller's article was described as showing the question still open. He argues to a verdict on the
  broader side. Also: it is a single-author article in a special issue, not a symposium, which the
  research file and a quiz explanation both had wrong.
- The Pilate stone is not the only inscription naming him; a ring from Herodium carries his name.
  It is the only one that names him with his office.
- Bea "helped draft the text" is not supported by the source cited, which places him at the council.
- The four senses were missing for the third time in this course, against another standing
  instruction. Present now, with the note that they are the fathers' method and normal Catholic and
  Orthodox practice, not a beginners' error.

### Neutrality

Perspectives checked: inerrantist, infallibilist, Catholic (both readings of Dei Verbum 11 and the
pre-conciliar magisterium), Orthodox, mainline and Barthian, Enns, confessional Reformed and
Anglican, traditional Jewish, historical-critical, and sceptical.

- **The title promised "what others make of it" and delivered the academy.** Judaism and Islam both
  hold developed doctrines of these books and neither appeared, in the lesson whose subject is what
  this book is. Both now have a paragraph in their own terms, with the honest note that a paragraph
  each is not enough and that World Religions treats them properly.
- The mustard-seed example labelled one reader by an emotional state ("a nervous believer") and the
  other descriptively. Now both are named by their reading.
- Chicago's Article III was quoted against the Confession of 1967 without naming Barth, which is
  what Article III was written against, leaving a reader with a disagreement with one party unnamed.
- Two over-hardenings fixed: "false to the field" on the Tel Dan reading, where a real minority
  disputes it, and "every critical scholar" on Pauline authorship of Romans.
- Two sneers at unnamed people, symmetric but still sneers, rewritten.
- The course declined to say where it stands on the one question the section says everything turns
  on, while promising to state its view "plainly". It now names the scope it holds and why.

### Pedagogy

- Assessment aside, the teaching was strong. The failures were placement and hiding: a 1,400-word
  doctrinal core with no interaction in it, an archaeology section with none, an answer printed in
  plain prose three lines under its own question (which the build had been flagging as a draft
  warning), and the exercise's answer key sitting inside the visible exercise block.
- All four fixed: a checkpoint on the four words, a predict before the archaeology columns, the
  Mark 1:5 problem converted to a predict, and the exercise key moved into a checkpoint.
- The exercise's claims were mostly recycled from the lesson. Three swapped for claims the lesson
  never sorts, including the resurrection, which is the case where the sort is hardest and most
  worth practising.

### Voice and media

- The paragraph-closing epigram was not a tic here, it was the default cadence: roughly 23 of 45
  body paragraphs closed on a one-line moral, usually an antithesis, which is what made a doctrine
  lesson read like a sermon. Twenty of them cut or flattened; the three that carry real content kept.
- The ideological-Turing-test line ("an inerrantist reading this should find...") appeared four
  times in the same grammar, which reads as the lesson grading its own homework. Two kept, two cut.
- Zero links in 12,000 words that quote seven confessional documents, six of them free online.
  All are now linked at the point of quotation.
- **The sort diagram was invisible to both SVG validators.** Every font-size and every hardcoded
  white fill sat on `<g>` wrappers rather than on `<text>`, and both checks in `scripts/build.mjs`
  matched only on the `<text>` tag, so 24 labels between 8.5 and 11 units passed clean while other
  lessons were flagged for the same defect. White on the gold band was also about 3.2:1. The chart
  is redrawn stacked, with every size on its own element, labels outside the swatches and no
  hardcoded colour. **The validator itself is fixed in the same commit** to resolve inherited
  font-size, which is what should have caught this; it now flags lessons 1, 11 and 12, all of which
  carry the same defect. 11 and 12 are the next two reviews; lesson 1 is a small follow-up.
- The Pilate figure hotlinked a 6.7 MB original. Swapped for the 118 KB thumbnail. Every
  `:::figure` in this course does the same thing, so a sweep is on the backlog.

### Unverified, and said so

- Cassuto, Kikawada and Quinn, and Garrett: named, nothing quoted, specific works still unconfirmed.
  The lesson's characterisation is the standard description of that literature, and the unsourced
  clause about ancient Near Eastern parallels has been deleted rather than left standing.
- Ware, *The Orthodox Church*: named only, edition unverified, nothing depends on it.
- Providentissimus Deus, Divino Afflante Spiritu and the 2014 PBC document are cited for the
  Catholic argument's background on standard dates and titles; not read in full for this lesson.

### Deliberately not fixed

- The reviewer proposed replacing the six-position comparison table's third column. Left alone: it
  is organised on the one variable the lesson is about, and shortening it would cost the exactness
  that makes it worth having.
- The suggestion to drop "Now" and "So" as paragraph openers wholesale was applied only where the
  sentence was doing drumroll work. Some of them are just how a person talks.

## Lesson 11, "Choosing a translation" — Stage 4 round 1, 2026-09-08

Four fresh-context subagents (depth and sourcing, fact-check, neutrality, voice and media with pedagogy),
plus a targeted Stage 1 research task to close gaps the reviews found. All applied in the same session.

### BLOCKING, applied

- **Three positions were described in their opponents' words**, which is the 3.7 failure this course
  keeps having to fix. The worst was that **Byzantine or Majority Text priority was run together with
  Received Text priority**, and the merged position was attributed to Wilbur Pickering, who argues
  Majority Text and is a critic of the Received Text. Both facts were sourced to Carson and White,
  two books written against the position and recorded in SOURCES.md as never opened. The lesson now
  has a section on Byzantine priority in its own advocates' words, from Robinson's 2001 essay and
  Hodges's 1968 and 1978 articles, all read: Robinson's "the Byzantine Textform is not the TR, nor
  need it be associated with the TR or those defending such in any manner", his description of the
  method as "reasoned transmissionalism", and his own rejection of the nose-counting caricature.
  The gap between the two texts is now quantified from three independent counts (Robinson's edition
  says over 1,800 and his collation lists 1,883; Wallace counted 1,838; Pickering predicts over
  1,500), and Hodges is quoted announcing that his edition deletes the Comma Johanneum and Acts
  8:37, which is the single fact that makes the distinction impossible to miss.
- **The complementarian case on gender language was a preface and three assertions.** The inclusive
  side got its corpus research and the other side got adjectives. The lesson now carries the argument
  its authors actually make: Poythress and Grudem's pure-generic against representative-generic
  distinction, their explicit denial that they are arguing for formal equivalence, Revelation 3:20
  and Psalm 1 as their worked cases, Grudem's published tallies, the 1997 Colorado Springs Guidelines
  with their signatories and the International Bible Society's later withdrawal, Strauss's reply that
  translation follows sense rather than form, the translation committee's own 2010 notes conceding two
  of the three main points, and the 2011 counts from both sides. All from sources read this session.
- **Catholic and Orthodox translation was absent**, in a lesson that then recommended eight Bibles,
  every one of them a sixty-six book Protestant edition, to a reader lesson 8 had taught that the
  Catholic canon has seventy-three. There is now a section on the Catholic and Orthodox shelf, and the
  Septuagint question is named where the source texts are introduced rather than left as a silent
  default. *Liturgiam Authenticam* is quoted correctly, since it is usually cited for the opposite of
  what it says: new translations "must be made directly from the original texts".
- **The strongest King James Only position is still owed.** It is named in the lesson, distinguished
  from the Trinitarian Bible Society and Dean Burgon Society positions, and the lesson says where to
  press it. What it does not yet have is a primary statement of the view in its own advocates' words,
  because the research for it was not in SOURCES.md and the pass to get it did not land in time. The
  lesson now says plainly that the course has not yet read a primary advocate of that position rather
  than summarising it from its critics, which is the honest interim state. **This is the one open item
  on the lesson.**
- **The chart carried 25 sub-legible labels and hardcoded gradient colours.** Nine versions packed
  into 380 viewBox units cannot hold legible type, so the layout is now vertical, in three bands, with
  every font-size on its own element and every colour tokenised.
- **Four links in 9,650 words**, none of them at the point where a document is quoted, in a lesson
  built entirely on quoting prefaces. Every preface it quotes is free online and is now linked where
  it is quoted.

### Facts corrected

- **The NIV preface does not say what was attributed to it.** The lesson credited it with the
  reasoning behind rendering *adelphoi* "brothers and sisters"; the words *adelphoi* and *brothers*
  do not occur in the preface, and the research file had already recorded that the committee's
  reasoning was seen only in secondary summaries. Reattributed, with the Collins figure kept because
  it is in the preface and correct.
- Erasmus's 1516 was the first printed Greek New Testament **to be published**; the Complutensian was
  printed two years earlier and held back. His own line about the edition is now quoted from Allen's
  edition of the letter, addressed to Nikolaus Ellenbog, about April 1516, with the note that the
  widely circulated Latin form is not his wording and that the second half of his sentence, a boast,
  is usually dropped.
- The Elzevirs printed the 1633 edition; Daniel Heinsius wrote the preface with the famous sentence.
- The NKJV note at Mark 16:9 does not begin "NU-Text"; it says the verses are bracketed in NU.
- Two ESV preface quotations were trimmed of the clauses that carry their argument. Restored, and the
  research file corrected, since the truncation was inherited from it.
- Amos 4:6 in the NET begins "But surely", which the lesson had dropped.
- "The Trinitarian Bible Society has published only the Authorised Version since its founding" could
  not be sourced; narrowed to what is verifiable.
- A quiz explanation had the critical editions built "from thousands of earlier" manuscripts, which
  overstates the early evidence in exactly the way this course warns against.

### Neutrality

Perspectives checked: Received Text priority in its two organised forms, Byzantine priority,
KJV-as-preserved-English, formal equivalence, functional equivalence, both sides of the gender
dispute, Catholic, Orthodox, and conservative Hebraists on *almah*.

- **The lesson worked the Received Text position's weakest case and none of its strongest.** 1 John
  5:7 got a whole section; 1 Timothy 3:16 and Mark 1:2, which is where its defenders think they are
  strongest, were absent. Both are now given, with the harder-reading answer, and the section says
  explicitly that the Comma is the case with least manuscript support and so not a fair sample.
- **The trade was framed throughout in the formal camp's terms**, as accuracy against readability.
  The functional camp's own claim, that meaning-based translation is the more accurate method, is now
  stated where the spectrum is introduced.
- **The publishers' own names for their own methods were all in the footnotes and none in the body.**
  "Essentially literal", "complete equivalence", "optimal equivalence", "as literal as possible, as
  free as necessary" are now in the text, with the observation that three of them are claiming the
  spectrum is a false choice.
- *almah* at Isaiah 7:14 was presented as settled on one side, with a single translation's note as
  support. It is now given as argued, with the conservative Hebraists' case stated.
- "Not a conspiracy" removed from the Connections summary: nobody in that camp describes their
  position that way, and the word belongs to their critics.
- "Paraphrase" was used as a category verdict. The lesson now distinguishes a paraphrase proper,
  which reworks an existing English version, from a free translation made from the original languages,
  and notes that Peterson resisted the label for that reason.

### Pedagogy and voice

- Four blocks in the first 1,600 words and one in the last 3,500. The 2,359-word stretch covering the
  hardest material asked nothing, and the one retrieval prompt inside it printed its answer in plain
  prose. That prompt is now a predict block, and a checkpoint was added after the gender section.
- Thirteen of 55 body paragraphs closed on a one-line moral, clustered three in a row. Six cut.
- The banned "by the end of this lesson" promise, and the same three-question routine performed twice
  56 lines apart. Both fixed.
- Three sentences over 100 words split, and a 127-word run of undated-purpose dates cut to the three
  that teach something, since the chart already carries them all.
- The lesson promised the reader could identify any Bible's text and method and never said where to
  look. There is now a short section on the preface, the copyright page and the footnote conventions.
- The reading-levels paragraph told the reader to treat as a hint a number the lesson had just refused
  to give. Replaced with a test they can run in two minutes.
- Quiz: three of five correct answers were the longest option and four of five were the only balanced
  one, so a test-wise reader scored full marks by shape. Three items replaced, and the quiz now runs
  to six with four applying to cases the lesson does not work.
- The 2 MB Wikimedia original swapped for the 960px thumbnail.

### Unverified, and said so

- Fee and Stuart chapter 2, the source of the spectrum placements, rests on the research file's record
  of a full read and could not be re-checked.
- Carson's *The Inclusive Language Debate* is quoted in this literature only by its opponents. The
  lesson does not quote it.
- The 1997 Zondervan and International Bible Society press releases survive here only inside other
  people's chronologies.
- Combs's article, recorded as read in full, no longer has a working URL at the address in the file;
  a working one was found this session and recorded.

## Lesson 12, "A way of reading you can keep" — Stage 4 round 1, 2026-09-08

Three fresh-context subagents (depth and sourcing, fact-check, voice and media with pedagogy).
All applied. Every one of the 21 URLs the lesson recommends was fetched and returns 200, which for a
lesson whose whole purpose is to send a reader to tools is the check that mattered most.

### BLOCKING, applied

- **Augustine was quoted saying what his correspondent said.** The sentence about pagans laughing at
  the Jonah story is the question put to him, not his reply. His actual answer, that either every
  miracle is incredible or there is no reason to disbelieve this one, is now what the lesson reports.
- **The lesson justified daily reading with the spacing literature, which does not support it.**
  Cepeda and colleagues measured distributed re-study of the same material; a linear reading plan is
  new material every day. The mechanism is now stated honestly, and the research is cited for the two
  places it does bear on: M'Cheyne's calendar, which really is spaced re-reading, and the log's demand
  for one observation a sitting, which is retrieval practice.
- **It launched a course project that did not exist.** Both assessments are now written, and the
  project follows the outline's spec and rubric.
- **Philippians 4:13 was quoted in the ESV's wording**, untagged, in a course that quotes the NET by
  design and may not quote the ESV at all under its licence. Corrected.
- **Two prompts printed their answers in plain prose**, which the build had been flagging as a draft
  warning. Both are now predict blocks.
- **Both charts failed.** The circles diagram had seven labels at 10 and 11 units; the reading-time
  chart had ten more inheriting 10 from a group wrapper, two labels hardcoded white, and two of its
  five bars drawn one pixel wide, which its own footnote admitted. Redrawn: nested frames for the
  first, two panels with their own stated scales for the second.
- **The Society for Old Testament Study was quoted against its own conclusion.** Its "too long to be
  a parable" sentence sits inside a paragraph that opens "Granted that the book is not a historical
  narrative", and the lesson used the quotation to support a conservative reply while omitting that
  premise and the reasons given for it. All of it is now on the page.

### Facts corrected

- "Four hundred and fifty years apart" is about 495; Nehemiah 8 is 445 BC and Acts 17 about AD 50.
- Philemon is one of a few letters occasioned from the writer's side, not the only one. The lesson's
  own footnote and lesson 7 both said "few"; only the body said "one".
- Nehemiah 8:7 has the Levites teaching while the people stood, not moving through the crowd.
- Two M'Cheyne quotations were attached to the wrong numbered advantages, and the "green pasture"
  phrase is from his preamble rather than his list.
- The New Testament in a year is about three minutes a day on Crossway's own rate, not six.
- The free NET reader is netbible.org; netbible.com is the publisher's shop.
- The DTS course does not state a duration and requires a sign-up, both of which the lesson now says.
- The course's own translation says Babylonians at Habakkuk 1:6, not Chaldeans.
- "Reading alone is not the ideal the Bible sets" was unsourced and is contradicted by Acts 8. Replaced
  with the sourceable claim about literacy and the cost of copies.
- `minutes: 45` understated the lesson by roughly a factor of three once its own reading assignments
  are counted. Now 150.

### Pedagogy and voice

- Twelve of 56 body paragraphs closed on a one-line moral, and five of the twelve were in the last
  three sections, which is the sermon register arriving exactly where a closing lesson is most at risk.
  Seven rewritten, including the motivational send-off and the four-beat closing listicle.
- Two stretches of 1,900 and 1,300 words asked nothing, the second covering all four misconceptions.
  Two checkpoints added, one of which turns the tools list from a resource roundup into a matching
  exercise, since objective 3 is about matching tool to question and nothing tested it.
- The tools list had one generic entry, and it was the one carrying two of the four questions, plus a
  claim about what four named study Bibles say at three passages, sourced to nothing. Narrowed.
- Quiz: correct answer was the longest option in three of six, one distractor was the silly kind 4.3
  forbids, and only half the items applied to unseen cases. Four items replaced.
- The 3.3 MB Wikimedia original swapped for the 148 KB thumbnail, and the three unlinked reading plans
  and four unlinked sources are now linked where they are named.

### Deliberately not fixed

- The suggestion to work an unworked book in the exercise's part 1 instead of Jonah was not taken.
  Jonah is the lesson's second worked example on purpose, and the project immediately after it sets the
  reader on a book the lesson does not work, with a note steering them away from the two it does. The
  transfer test is met by the project rather than by the exercise, which is the right place for it.
- The proposed chart of the four plans' daily cost was not added. The plans section is a decision, and
  four italic-lead paragraphs with the daily cost in each is enough; another chart would be the third
  in one lesson.

### Still owed, recorded rather than fixed

- Fee and Stuart's *Book by Book* is characterised in the body from the publisher's description. The
  claim is now hedged, but the honest fix is for someone to open the book, since its four headings are
  the pattern the project asks learners to use.
