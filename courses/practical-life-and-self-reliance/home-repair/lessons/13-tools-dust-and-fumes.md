---
title: "Tools, dust and fumes: a small kit used safely"
minutes: 95
objectives:
  - >-
    Explain how misuse and poor upkeep turn an ordinary hand tool into an injury, using OSHA's
    examples, and apply OSHA's power-tool precautions to a described job
  - >-
    Choose respiratory protection for a dusty job (FFP2 or FFP3 in the UK, an N95 in the US), and
    explain why a dust mask does nothing against solvent vapour and fails over a beard
  - >-
    Put the controls for a dusty job in HSE's order (less dust, then water or extraction at the
    tool, then a respirator), and say what a solvent job needs instead
  - >-
    Apply the London Fire Brigade's charging advice to a cordless tool's battery and charger, and
    say what to do with one that swells
quiz:
  - q: >-
      In a cottage in Ceredigion, the wooden handle of a claw hammer has a crack running back from
      the head. Its owner has one nail to knock in before the weekend. On OSHA's hand-tool booklet,
      what's the right call?
    options:
      - Use it for the one nail with light taps, since cracks only matter when you swing it hard
      - Bind the crack with tape first, since the booklet's worry is splinters in your palm
      - Soak the head end in water overnight, so the wood swells tight on the head again
      - Put it aside, since OSHA says a cracked handle can let the head fly off and hit someone
    answer: 3
    explain: >-
      OSHA's booklet says that if a wooden handle on a tool such as a hammer "is loose, splintered,
      or cracked, the head of the tool may fly off and strike the user or other employees", and its
      five basic rules include not using damaged tools. Light taps are still use of a damaged tool,
      and the booklet sets no safe force. Its concern is the head flying off, not splinters, so tape
      doesn't answer it. And no source this course read suggests soaking a handle: that's a remedy
      with nothing behind it here.
  - q: >-
      In a US townhouse garage, someone's jigsaw blade has snapped halfway through a cut, and the
      saw is still plugged in on the bench. By OSHA's power-tool precautions, what comes before
      the new blade goes in?
    options:
      - Unplug the saw, since OSHA says disconnect tools when changing blades, bits and cutters
      - Lock the trigger off, since a locked switch is OSHA's condition for changing a blade
      - Keep a finger clear of the trigger, since OSHA's worry is starting it by accident
      - Let the motor cool for ten minutes, since OSHA's concern is a burn from the blade clamp
    answer: 0
    explain: >-
      OSHA says: "Disconnect tools when not using them, before servicing and cleaning them, and when
      changing accessories such as blades, bits, and cutters." Disconnect means the plug, not the
      switch. A trigger lock isn't in OSHA's list as a substitute. Keeping fingers off the switch is
      OSHA's rule for carrying a plugged-in tool, a different moment. And the booklet gives no
      cooling time for a blade change.
  - q: >-
      A joiner in Moray with a thick two-week beard puts on a new FFP3 disposable respirator to cut
      a stack of bricks. The mask is marked correctly and he does HSE's pre-use check. What does
      HSE's guidance say about the protection he gets?
    options:
      - Close to the full factor of 20, because FFP3 is the highest of HSE's three FFP grades
      - The full factor, provided the check showed no leak and his glasses stayed clear
      - Very little, since HSE says a face-fit mask is useless with a beard or thick stubble
      - About half the factor, since HSE says a beard halves an FFP3's protection factor
    answer: 2
    explain: >-
      HSE says a respirator that depends on face fit "is useless if you have a beard or thick beard
      stubble", and its construction dust sheet says anyone using a tight-fitting mask "also needs
      to be clean shaven". The factor of 20 depends on the seal, so the grade on its own doesn't
      deliver it. The pre-use check finds leaks you can detect; it doesn't make a beard seal. And no
      HSE page read here gives a reduced factor for a beard: "useless" is its word.
  - q: >-
      Before varnishing a floor in a closed bedroom in Hawaii, a homeowner plans to wear an N95
      because the tin says the vapour is harmful. What do NIOSH and EPA, as this lesson read them,
      point to?
    options:
      - The N95 is fine, since NIOSH says it filters at least 95% of what's in the air around you
      - The N95 stops particles, not vapour, so follow EPA and work outdoors or with windows open
      - A second N95 over the first, since two layers of filter would stop most of the vapour
      - The N95 works for vapour only if it's fit tested, as OSHA's rule for workers requires
    answer: 1
    explain: >-
      NIOSH says an N95 "Filters at least 95% of airborne particles", and that filtering facepiece
      respirators protect by filtering particles out of the air. A vapour isn't a particle, and
      HSE's dust mask page and its wood dust sheet both say dust masks give no protection against
      gases and vapours. EPA's advice for a product whose label says use in a well-ventilated area is to go
      outdoors or use an exhaust fan, and otherwise open windows. Doubling the mask or fit testing
      it changes nothing about what the filter is for.
  - q: >-
      In a croft house on Shetland, the battery pack of a cordless hedge trimmer has started to bulge
      at one end since last summer. The charger is the one it came with. On the London Fire
      Brigade's advice, what's the step?
    options:
      - Charge it only when someone's in the room, and keep it on a hard surface while it charges
      - Run it flat in the trimmer to use up the charge, then take it to be recycled with the rest
      - Open the casing to check the cells, then refit it if none of them are damaged or leaking
      - Stop charging, turn it off, don't open the casing, and contact the maker or a technician
    answer: 3
    explain: >-
      LFB's steps for a battery that starts swelling or expanding are "Stop charging immediately",
      "Turn off the device", "Do not remove the casing or tamper with the equipment", and "Contact
      the manufacturer or certified technician". Supervised charging on a hard surface is still
      charging it. Running it flat is still using it, and no source here says to. Opening the
      casing is the one thing LFB says in terms not to do. Having the original charger is one of
      LFB's tips, but it doesn't make a swollen battery safe.
  - q: >-
      As general information: a US neighbour offers you a half-used tin of methylene chloride paint
      remover from before 2019. What does EPA's page on the chemical say about it?
    options:
      - Nothing applies to a tin bought before 2019, so it's fine to use as long as it's outdoors
      - That use is now legal only with an N95 and a fan, under the rule EPA finalised in 2024
      - Its supply for consumer paint removal was banned in 2019, and EPA asks you to stop using it
      - The 2024 rule lets householders finish tins bought before the ban, as long as it's outdoors
    answer: 2
    explain: >-
      EPA's page says the 2019 rule prohibited making and distributing it, retailers included, for
      consumer paint and coating removal, and that "EPA is
      encouraging all consumers to stop using methylene chloride products that they may have already
      purchased for paint and coating removal", sending leftovers to state and local solid waste
      agencies for disposal instructions. EPA's older indoor air page still says to use such products
      outdoors where possible, but that page is general and the rule page is the one that deals with
      the ban. Nothing on the rule page lets householders finish old tins, and no EPA page read here
      offers an N95 as the answer: a dust mask stops particles, not vapour.
---

Most injuries from a small tool kit don't come from the tool being dangerous. They come from the
tool being used for the wrong job, or in the wrong state: a chisel used as a screwdriver, a hammer
with a cracked handle, a saw with its guard taken off. The dust is slower. What you breathe
while cutting a paving slab or sanding a board can do its damage over years, long after the job is
forgotten. This lesson gathers the few tools the earlier lessons asked for, then takes each hazard
in turn: the tool itself, the dust, the fumes, and the battery on charge.

:::callout Before you pick up a tool
This course is education, not a trade qualification, and not legal advice about your own home. It teaches no gas work and no wiring. If you smell gas in Great Britain, open doors and windows, turn the gas off at the meter if you can do it safely (not if the meter is in a cellar or basement), and call 0800 111 999; in Northern Ireland the number is 0800 002 001. In the US, leave straight away and call 911 or your gas utility once you're outside. In either country, don't light a flame or touch an electrical switch. Isolate the water or power before you open anything up, and if a breaker, RCD or GFCI keeps tripping, leave it off and call an electrician. Which jobs the law leaves to a registered or licensed trade differs between England, Wales, Scotland, Northern Ireland and each US state, and it changes.
:::

"Isolate the power before you open anything up" has a tool-sized meaning in this lesson: a power
tool is unplugged, or its battery taken out, before you change a blade or a bit, as the section on
power tools sets out. The box stays shut, as it did in lessons 5 and 6, and the two facts that go
with it still hold. CPSC, the US Consumer Product Safety
Commission, says panel boxes "are usually electrically live, even with the main circuit breakers
turned off".[13] And HSE's guidance for people who work on electrical systems does not recommend
"non-contact devices such as 'volt sticks'" for proving anything dead.[14]

Where this lesson touches the law, this is general information, not legal advice. The rules differ
between England, Wales, Scotland and Northern Ireland, and between US states and cities, and they
change.

## Whose rules these are

This section states law. This is general information, not legal advice.

Almost every source in this lesson was written for people at work. OSHA, the US Occupational Safety
and Health Administration, wrote its booklet [*Hand and Power Tools*](https://www.osha.gov/sites/default/files/publications/osha3080.pdf) for employers and employees
under its workplace standards.[1] HSE, the Health and Safety Executive, which regulates workplace
health and safety in Great Britain, wrote its [construction dust sheet](https://www.hse.gov.uk/pubns/cis36.pdf) to tell "employers what they
need to know to prevent or adequately control construction dust risks", and its [wood dust sheet](https://www.hse.gov.uk/pubns/wis23.pdf) for
woodworking employers.[3][4] OSHA's respirator and silica rules bind employers.[8][10] And HSE says
of its own sheet: "Following the guidance is not compulsory, unless specifically stated, and you
are free to take other action."[3]

So none of these rules is written for a householder's own DIY, and this course read no rule on
masks or goggles at home that is.
The advice is the other half. These are the best-specified practice anyone has written for
cutting, drilling and sanding, and the dust does not know whether you are being paid. The pattern is
the one lesson 1 set out, the law allowing more than the advice, and the label follows the advice.
Where this lesson says a job is **Yours, on conditions**, the conditions come from these workplace
sources, applied at home as this course's reading.

## A small kit, chosen by the jobs

This course does not give a shopping list, and it names no brand. The kit is whatever the jobs in
these lessons asked for, and the list below is this course's, drawn from the methods it has already
taught. Most of the tools are there because a named source's method used them; the torch is the
course's own.

| Tool | The job, and whose method | Lesson |
|---|---|---|
| A torch | Looking at shut-offs, plugs and leads in poor light (this course's) | 1, 6 |
| A bucket | WaterSafe's flow test at the kitchen tap | 2 |
| A spanner and pliers | Southern Water's washer change on a UK tap | 3 |
| Rubber gloves, a flat-bottomed sink plunger, a bucket, adjustable pliers | WaterSafe's blocked sink, with the trap last | 4 |
| A radiator key, a cloth, gloves | Worcester Bosch's and Vaillant's bleeding method | 7 |
| A candy or meat thermometer | CPSC's check of the water at the hot tap | 8 |
| Eye protection, a dust respirator, ear protection | This lesson, for drilling, cutting and sanding | 11 |

North Seattle College's beginners' home repair course asks each student to bring "eye goggles, a
respiratory mask, and ear protection".[15] That's one adult-education course's list, not a
standard, but it is the same three this lesson adds.

What the list leaves out matters as much. It has no volt stick, because HSE doesn't recommend one
for proving anything dead, and no tool for the inside of the box, because the box is not yours.[14]
Woodworking and making things belong to Tools and Making Things, later on the Core, and this course
does not teach them.

## Hand tools: how an ordinary tool hurts

:::predict A kitchen knife that's gone dull, and one that's sharp. Before reading on: which does OSHA say is the more hazardous, and do you think it gives a reason?

The dull one. OSHA's booklet says: "Knives and scissors must be sharp; dull tools can cause more
hazards than sharp ones."[1] It gives no reason, and this course won't supply one. Notice the scope,
too: the sentence is about knives and scissors, and the rest of the booklet makes the general point
through maintenance, as the next paragraphs show.
:::

OSHA's booklet puts the whole hand-tool problem in one sentence: "The greatest hazards posed by
hand tools result from misuse and improper maintenance."[1] Its examples are worth reading as a
list of habits to lose:

- "If a chisel is used as a screwdriver, the tip of the chisel may break and fly off, hitting the
  user or other employees."
- "If a wooden handle on a tool, such as a hammer or an axe, is loose, splintered, or cracked, the
  head of the tool may fly off and strike the user or other employees."
- "If the jaws of a wrench are sprung, the wrench might slip."
- Impact tools such as chisels and wedges with "mushroomed heads" (spread at the struck end, like
  a mushroom's cap) "might shatter on impact, sending sharp fragments flying toward the
  user or other employees."[1]

Look at what the four have in common. Three are about the tool's state (a cracked handle, sprung
jaws, a mushroomed head) and one about its job (a chisel doing a screwdriver's work). In each, the
hazard is something leaving the tool at speed or the tool leaving the work. OSHA's booklet
describes what happens, not the physics of why, and this lesson stays with its description.

The booklet then gives five basic rules for hand and power tools alike. Four are about the tool:
"Keep all tools in good condition with regular maintenance." "Use the right tool for the job."
"Examine each tool for damage before use and do not use damaged tools." "Operate tools according to
the manufacturers’ instructions." The fifth is about you: "Provide and use properly the right
personal protective equipment."[1]

The booklet is a US workplace document, written for employers. That these rules carry over to a
kitchen drawer is this course's reading, and it is not a large leap: nothing in the four examples
depends on who pays for the hammer. Using a sound hand tool for its own job is **Yours**. A tool
with a cracked handle, sprung jaws or a mushroomed head is out of use until it's repaired or
replaced.

## Power tools: the precautions

OSHA's list of general precautions for power tools is short, and nearly every line is a habit
rather than a piece of equipment:[1]

- "Never carry a tool by the cord or hose."
- "Disconnect tools when not using them, before servicing and cleaning them, and when changing
  accessories such as blades, bits, and cutters."
- "Keep all people not involved with the work at a safe distance from the work area."
- "Secure work with clamps or a vise, freeing both hands to operate the tool."
- "Avoid accidental starting. Do not hold fingers on the switch button while carrying a plugged-in
  tool."
- Dress for it: "Loose clothing, ties, or jewelry can become caught in moving parts."
- And take a damaged portable electric tool out of use and tag it "Do Not Use".

On guards, OSHA is flat: "Safety guards must never be removed when a tool is being used."[1] For a
powered grinder it adds: "Always use eye or face protection."[1]

"Disconnect" is the plug, or the battery on a cordless tool, not the tool's own switch. That's the
plain meaning of OSHA's word, and it is the same shape as lesson 6's rule that a plug is opened only
when it's out of the socket.

Electrical Safety First (ESF), a UK charity that campaigns on electrical safety, adds three lines
for a British reader using power tools at home: "Use an RCD or plug-in RCD." "Check leads and plugs
on power tools." And if the lead or plug shows damage, "for example, frayed wires", "get the
equipment professionally repaired before using it or get it replaced."[5] Lesson 5 explained what an
RCD does and lesson 6 what a plug-in RCD is for; neither is retaught here.

Using a sound power tool for its own job, by the maker's instructions, with the guard on, is
**Yours, on conditions**, and OSHA's list is the conditions.

## Eyes and ears

OSHA's booklet says that eye protection is needed with a powered grinder, and that "safety goggles
and gloves" are the kind of equipment needed against the hazards of hand tools at work.[1] This
course did not read a standard for eye protection in either country, so it names none. In general
terms: wear eye protection whenever a tool can throw something (drilling, cutting, grinding,
chiselling, knocking in masonry nails), and read the marking and the maker's information on the
pack to see what it is made for.

For noise, OSHA's booklet says "Noise is another hazard associated with pneumatic tools", and asks
for hearing protection with noisy tools such as jackhammers.[1] This course read no source giving a noise level
for household power tools, so it gives no number. North Seattle College's kit list includes ear
protection, and so does this one.

If something does get into an eye, First Aid and CPR lesson 10, "Burns, cuts and small injuries",
covers it: don't rub it, and what to do next in each country. This lesson doesn't reteach it.

## Dust: why it's slow, and why it matters

HSE's construction dust sheet opens: "Construction dust is not just a nuisance; it can seriously
damage your health and some types can eventually even kill."[3] It names three kinds:[3]

- **Silica dust**, made when working on silica-containing materials "like concrete, mortar and
  sandstone";
- **wood dust**, "created when working on softwood, hardwood and wood-based products like MDF and
  plywood";
- "other ‘general’ dust", from materials with little or no silica, the commonest being gypsum (in
  plasterboard), limestone, marble and dolomite.

### Silica

HSE says silica is in most rocks, sand and clay, and in "bricks, tiles, concrete and mortar". Cutting,
drilling, grinding and polishing them makes dust, and "Some of this dust is fine enough to get deep
into your lungs." That fine dust is "too fine to see with normal lighting". And: "Silica is the
biggest risk to construction workers after asbestos."[6] On silicosis, HSE's page is blunt about the
thing that makes it different from a cut: "The effect continues to develop after exposure has
stopped and is irreversible."[6]

OSHA says the same from the American side. Respirable crystalline silica is "very small particles at
least 100 times smaller than ordinary sand you might find on beaches and playgrounds", and it "is
created when cutting, sawing, grinding, drilling, and crushing stone, rock, concrete, brick, block,
and mortar". OSHA lists silicosis as "an incurable lung disease that can lead to disability and
death", with lung cancer, COPD and kidney disease beside it.[7]

That is the mechanism, as far as these sources give it: the dust that harms you is the dust you
can't see, it goes deep, and the damage is slow and one-way. So a job that looks clean in daylight
isn't evidence of anything.

### Wood dust

HSE's wood dust sheet, written for woodworking employers, says "All wood dust is a substance
hazardous to health because it can cause serious non-reversible health problems", and names asthma,
dermatitis, and irritation to the eyes, nose and throat. "Hardwood dust can also cause a rare type of
nasal cancer."[4] Its account of which dust matters is the same as the silica pages': "The biggest
risk is from fine dust, as you can breathe this deep into your lungs where it will do the most
damage."[4]

And on cleaning up: "Do not use compressed airlines or dry sweeping, particularly on clothing, as
these will just create dust clouds and redistribute the dust."[4] OSHA's silica rule tells employers
the same about dry sweeping and compressed air on a building site.[8]

### What to do about it, in HSE's order

HSE's sheet puts the controls in an order, and the order is the lesson:[3]

1. **Stop or reduce the dust** before you start. HSE: "Use different materials, less powerful tools
   or other work methods." Fewer cuts, materials the right size, a hand method instead of a power
   tool.
2. **Stop it getting into the air.** Water, "enough water supplied at the right levels for the whole
   time that the work is being done", and HSE adds: "Just wetting the material beforehand does not
   work." Or on-tool extraction, a hood on the tool connected to an extraction unit of the right
   class, and "Don’t just use a general commercial vacuum."
3. **Then respiratory protection.** "Remember: RPE is the last line of protection."

HSE also names where you work as a risk factor: "the more enclosed a space, the more the dust will
build up."[3] A garage with the door down is an enclosed space.

For a householder drilling a few holes, HSE's table has a row that fits: "Occasional short-duration
drilling with hand-held rotary power tools". Its controls are equipment that stops dust getting into
the air where possible, from a dust collector at the bit or a cordless extractor on the drill for
smaller bits, and otherwise a respirator with a protection factor of 20.[3] The next section says what
that factor means.

:::checkpoint Three jobs in one weekend: sanding an MDF shelf with a power sander, drilling four holes in a brick wall for a curtain rail, and sweeping up the garage floor afterwards with a stiff broom. Which kind of dust does each make, on HSE's three kinds, and what's wrong with the third job as described?

The MDF sanding makes wood dust (HSE names MDF). The brick wall makes silica dust (HSE lists brick).
The sweeping moves whatever settled from both, and dry sweeping is exactly what HSE's wood dust
sheet says not to do, because it throws the dust back into the air.[4] HSE's order for the first
two: less dust first, then extraction at the tool, then a respirator.[3]
:::

## Masks: what they stop, and what they don't

In the UK, the disposable dust respirators HSE describes are marked to a standard and graded FFP1,
FFP2 or FFP3. [HSE's page on dust masks](https://www.hse.gov.uk/agriculture/dustmasks.htm): "FFP1, FFP2 and FFP3 respirators can reduce the amount of dust you breathe by
factors of 4, 10 and 20 respectively."[2] A factor of 20 means you breathe a twentieth of the dust in
the air. HSE's construction dust sheet sets the general level for construction dust at that factor
of 20, and names the FFP3 disposable mask as the common way to get it; its wood dust sheet says the
same for woodworking.[3][4] HSE's page on dust masks has no date, points readers to guidance due "in
autumn 2005", and was written for farm work, so it's old. Its factor of 20 for FFP3 matches the
2020 construction sheet's.

In the US, NIOSH, the National Institute for Occupational Safety and Health, certifies respirators.
An N95 "Filters at least 95% of airborne particles. Not resistant to oil."[9] OSHA's respirator
rule for workers gives the class that includes filtering facepieces an assigned protection factor
of 10, and adds that its factors "are only effective when the employer implements a continuing,
effective respirator program as required by this section (29 CFR 1910.134), including training, fit
testing, maintenance, and use requirements."[10]

Those are two different measures. NIOSH's 95% is what the filter stops. OSHA's 10 is what a worker
wearing that class of mask is assigned inside a fit-tested programme. And HSE's factors are for the
UK grades. This course does not convert one into another, and neither should you: in the UK, read
the FFP grade; in the US, read the N95 and the NIOSH approval.

### The "dust mask" that isn't one

Some masks sold for dust are not respirators at all. HSE calls them nuisance dust masks or comfort
masks: they carry no standard marking, often have only one head strap, and their packaging may say
"This product does not provide respiratory protection", or something similar.[2] HSE's wood dust sheet
is plainer: "Nuisance dust masks do not filter out wood particles and hence provide no protection
against wood dust, so they should not be used."[4] In the US, NIOSH keeps a Certified Equipment List
for checking whether a respirator is NIOSH approved.[9]

### Why a beard defeats it

The mask works by sealing to your face, so the air you breathe comes through the filter and not
round the edge. HSE: "A respirator that depends on face fit is useless if you have a beard or thick
beard stubble."[2] Its wood dust sheet gives the mechanism: "Facial hair or glasses tend to lift the
respirator off the face and permit inward leakage of contaminated air."[4] Its construction sheet:
anyone using a tight-fitting mask "also needs to be clean shaven."[3] OSHA's rule for workers says
the same in its own terms: an employer may not let anyone wear a tight-fitting respirator who has
"Facial hair that comes between the sealing surface of the facepiece and the face or that interferes
with valve function".[10] For a bearded worker, HSE's wood dust sheet points to a hood or helmet
type instead, which doesn't depend on a face seal.[4]

:::predict A mask marked FFP3, worn properly, clean shaven, and fitted with its straps and nose strip adjusted. The job is stripping old varnish with a solvent stripper indoors. What protection do you think the mask gives against the fumes?

None. HSE: "FFP1, FFP2 and FFP3 are designed to filter out dust only. They must not be used where
there is an oxygen-deficient atmosphere or harmful gases and vapours."[2] Its wood dust sheet says
dust respirators are not suitable "against gases or vapours (eg from paint spraying) as they will not
provide any protection."[4] NIOSH describes filtering facepiece respirators as protecting "by
filtering particles out of the air".[9] A vapour isn't a particle. The mask's grade measures dust; it
says nothing about fumes.
:::

### The check you do every time

HSE says a proper fit test is the only way to know the seal is good enough, and that it is the only
way of meeting the legal requirements at work.[2] At home, the check you can do is HSE's pre-use
check, for a disposable respirator of the kind it describes: "You should carry out a pre-use check
every time you put on your respirator." Fit it by the maker's instructions, with the straps and the
nose strip adjusted, "then hold the mask in place and breathe in or out sharply. You should readjust
the respirator if you detect any leakage around your face or your glasses steam up when you breathe
out."[2] NIOSH's version for filtering facepieces is the maker's donning instructions in or on the
packaging.[9] Wearing a marked, well-fitted respirator is **Yours**, and the pre-use check is part of
wearing it.

## Fumes: solvents, strippers and the one chemical that was banned

This section states law. This is general information, not legal advice.

[EPA](https://www.epa.gov/indoor-air-quality-iaq/volatile-organic-compounds-impact-indoor-air-quality), the US Environmental Protection Agency, counts paints, paint strippers and other solvents among
the sources of volatile organic compounds (VOCs) in a home, and says "Concentrations of many VOCs are
consistently higher indoors (up to ten times higher) than outdoors."[11] Stripping is the extreme
case: "During and for several hours immediately after certain activities, such as paint stripping,
levels may be 1,000 times background outdoor levels."[11] The short-term effects EPA lists include
headaches, loss of coordination and nausea.[11]

EPA's advice, for a product whose label says to use it in a well-ventilated area: "go outdoors or in
areas equipped with an exhaust fan to use it. Otherwise, open up windows to provide the maximum
amount of outdoor air possible."[11] Buy small: "buy only as much as you will use right away". And
don't bin the leftovers: "Do not simply toss these unwanted products in the garbage can." Use a
household hazardous waste collection if your area runs one.[11] That is US advice. For the UK, this
course read no official page on solvents at home, so it says only: read the tin, and follow the
maker's instructions. Lesson 20, "Walls and paint: filling, preparing and painting", comes back to
paints and their labels.

The mask, as the last section showed, is no help here. Air is.

### Methylene chloride

One solvent needs its own paragraph. EPA's indoor air page says of methylene chloride, found in paint
strippers, adhesive removers and aerosol spray paints, that it "is converted to carbon monoxide in the
body and can cause symptoms associated with exposure to carbon monoxide".[11] Lesson 11, "Carbon
monoxide and the alarms that warn you", is about carbon monoxide from burning fuel. This is
carbon monoxide made in the body from a vapour, with no flame anywhere.

The US law has moved. [EPA's page on the chemical](https://www.epa.gov/assessing-and-managing-chemicals-under-tsca/risk-management-methylene-chloride) says that in March 2019 it issued a final rule to
prohibit the manufacture, processing and distribution of methylene chloride for consumer paint and
coating removal, "because of the acute fatalities that have resulted from exposure to the chemical in
consumer paint and coating removal".[12] In April 2024 it finalised a wider rule, which "Prohibits
manufacturing, processing and distribution of methylene chloride for all consumer uses."[12] That's
federal law, and this course read EPA's own summary of it, not the rules' text.

What EPA asks of a householder with an old tin: "EPA is encouraging all consumers to stop using
methylene chloride products that they may have already purchased for paint and coating removal." For
disposal: "Please consult your state and local government solid waste agencies to obtain proper
disposal instructions for leftover or unused paint and coating removal products."[12]

EPA's two pages don't read the same. The indoor air page, updated in June 2026, still says to "Use
products that contain methylene chloride outdoors when possible; use indoors only if the area is well
ventilated."[11] The rule page is the one about the ban, and it asks consumers to stop.[12] This course
does not explain the difference, because EPA does not. For paint removal, it follows the page that
deals with the rule.

This course read no UK rule on methylene chloride, so it says nothing about the UK position. If you
find an old tin in Britain, the label and your council's hazardous waste service are the places to
ask.

## A shelf sanded in Denbighshire

Take a simple job first. In a flat in Denbighshire, someone has cut a new MDF board for a shelf and
wants to sand its edges smooth with a corded power sander. The steps are in the order this lesson's
sources give.

1. **Less dust first.** HSE's table has a row for "Sanding wood with power tools", and the first way
   it gives to limit the dust is "Using ‘pre-finished’ materials".[3] If a ready-finished board
   would do, that is the job with the least dust. Here the board is already cut, so on to the next
   step.
2. **The tool.** ESF: check the lead and plug, and use an RCD or a plug-in RCD.[5] OSHA: clamp the
   board so both hands are on the sander, keep other people clear, and unplug it to change the
   sanding sheet.[1]
3. **Where.** HSE says dust builds up the more enclosed the space is, and suggests enclosing the work
   and keeping people away from it.[3] Outside, or a room with the door shut and nobody else in it,
   is the course's reading of that for a flat.
4. **Extraction, then the mask.** HSE's controls for this row are on-tool extraction to an H or M
   class extraction unit "and" a respirator with a factor of 20: both, not either.[3] A sander's own
   dust bag isn't what HSE describes, and HSE says not to rely on a general commercial vacuum, so
   without the right unit the respirator carries more of the job. An FFP3, pre-use checked, and eye
   protection.[2]
5. **Clean up.** No dry sweeping and no blowing the dust off: HSE's wood dust sheet says both "just
   create dust clouds and redistribute the dust".[4]

Sanding a board this way is **Yours, on conditions**, and the conditions are HSE's, OSHA's and ESF's.
What makes it simple is that the material is known and new. The next case isn't simple.

## A garage in New Mexico

This one has three problems, and you're going to find them before you read them.

A man in New Mexico, with a full beard, wants to lay a short path. On a cold, windy Saturday he
cuts concrete pavers in his attached garage with the big door pulled most of the way down, using a
borrowed handheld masonry saw, cutting dry. He's wearing a white one-strap mask from a multipack
whose box says it's for nuisance dust. When the path is done, he plans to strip an old door in the
same garage with a tin of paint remover he found on the shelf. The label says it contains methylene
chloride.

:::checkpoint Find three separate problems, and for each, say which source's words answer it. Write them down before you open this.

**1. The silica, cut dry and indoors.** Concrete is one of the materials OSHA names: cutting and
sawing it makes respirable crystalline silica.[7] OSHA's silica rule for construction work gives
employers a table of controls, and for a handheld power saw of any blade diameter the first is "Use
saw equipped with integrated water delivery system that continuously feeds water to the blade". Even
with water, OSHA requires a respirator with a protection factor of 10 when the saw is used indoors or
in an enclosed area, however short the job.[8] That rule binds employers, not him, but it is the
clearest measure anyone has written of how much dust this job makes. A garage with the door down is
the enclosed case. The borrowed saw also wants OSHA's checks: look it over for damage first, and its
guard stays on.[1]

**2. The mask, twice over.** A one-strap mask with no approval marking is the "nuisance dust" kind,
not a respirator: in the US, check for NIOSH approval.[9] And even an N95 wouldn't seal over a full
beard: OSHA's rule for workers bars tight-fitting respirators over "Facial hair that comes between
the sealing surface of the facepiece and the face".[10]

**3. The stripper.** No particle mask of any kind stops vapour; NIOSH describes these respirators as
filtering particles.[9] The vapour levels EPA describes during stripping ("1,000 times background
outdoor levels") are the reason for air, not a mask.[11] And this chemical is methylene chloride:
EPA's page says that since 2019 it has been prohibited to make or distribute it, retailers included,
for consumer paint and coating removal, and it asks consumers to stop using what they already have and to ask state or local solid waste agencies how to dispose of
it.[12] The course's answer is not to use it at all, which is EPA's.

What would make the paver job **Yours, on conditions**, on these sources: cut outside, with the
fewest cuts; water fed to the blade if the saw has it (OSHA) or a different method; and a
NIOSH-approved respirator on a clean-shaven face. That last is a choice he may not want to make. The
alternatives are a respirator that does not depend on a face seal (HSE's wood dust sheet names hoods
and helmets; this course read no US page on them) or someone else making the cuts.[4]
:::

## Batteries and chargers

Cordless tools run on lithium-ion packs. The [London Fire Brigade](https://www.london-fire.gov.uk/safety/lithium-batteries/) (LFB) says that in London "On
average there’s a fire from a lithium-ion battery in an e-bike or e-scooter every two days", and that
"Many of these fires happen in homes while batteries are charging."[16] That is London's figure, and
about e-bikes and e-scooters. This course read no US battery figure and gives none.

LFB's advice on batteries and chargers is written about phones, laptops and other devices, not tool
packs. Applying it to a cordless drill's battery is this course's reading, and the tool's own manual
comes first where it says something different. LFB says batteries can be a fire risk "when
over-charged, short-circuited, submerged in water or if they are damaged".[17] Its tips:[17]

- "Always use the charger that came with your phone, tablet, e-cigarette or mobile device", and if you
  replace it, a genuine one from a supplier you can trust.
- Avoid storing, using or charging batteries at very high or low temperatures.
- Protect them from being crushed, punctured or immersed in water.
- "Don’t leave items continuously on charge after the charge cycle is complete".
- "Never cover chargers or charging devices".

If a battery starts swelling or expanding, LFB's steps are: "Stop charging immediately", "Turn off the
device", "Do not remove the casing or tamper with the equipment", and "Contact the manufacturer or
certified technician".[17] A swollen pack is **Call someone**: the maker, not a screwdriver.

## What people get wrong

**"Any dust mask will do."** Nuisance masks aren't respirators, and a face-fit respirator is useless
over a beard (HSE).[2]

**"My dust mask covers the stripper fumes too."** Dust masks filter dust only (HSE); fumes need air
(EPA).[2][11]

**"A blunt blade is the safe one."** OSHA says the opposite for knives and scissors.[1]

**"If I can't see the dust, it isn't there."** The silica that matters is "too fine to see with normal
lighting" (HSE).[6]

**"Wet the slab first and it won't be dusty."** HSE: "Just wetting the material beforehand does not
work." Water has to be there for the whole cut.[3]

**"The switch is off, so I can change the blade."** OSHA says disconnect the tool, which is the plug
or the battery, not the switch.[1]

## Practice

:::exercise Your kit, and one dusty job planned
Take 25 minutes over this, with a notebook. It's all looking and planning. Don't drill, cut, sand or
scrape anything for this exercise, and in particular nothing that might be asbestos or old paint.
Don't open a consumer unit, panel, socket or fitting, and don't use a volt stick or tester on
anything. If you rent, this is planning only, and any real job waits until it's your home to repair
or your landlord agrees.

1. **The jobs, then the kit.** List the jobs from lessons 1 to 8 you expect to do in the next year,
   and next to each, the tool its source's method uses (the table in this lesson is the starting
   point). Note what you already own and anything in poor condition: a cracked handle, sprung jaws,
   a frayed lead, a mushroomed chisel. Those go out of use.
2. **One dusty job, on paper.** Pick one real job that will make dust: cutting a slab, sanding a
   board, drilling masonry. Name the kind of dust on HSE's three kinds. Then write the controls in
   HSE's order: how you could make less of it, whether water or extraction at the tool is possible,
   and which respirator comes last. Add eye protection, where you'll work (outside, or how enclosed),
   and how you'll clean up without dry sweeping.
3. **The mask, if you have one.** Read its marking: an FFP grade in the UK, an N95 and a NIOSH approval
   in the US, or neither. If it's a respirator, do HSE's pre-use check once in front of a mirror. If
   you have a beard, note that the check can't fix the seal.
4. **Batteries and chargers.** For each cordless tool, phone or e-bike charged at home: is the charger
   the one it came with? Where does it charge, and is anything covering it? Is anything left on
   charge after it's full? Any swelling? A swollen battery goes on the stop list, and the maker gets a
   call.
:::

:::checkpoint Check your plan against three questions. In step 2, did your controls start with less dust, or with the mask? If your job was indoors, what did you do about the enclosed space? And in your battery audit, which of LFB's tips does your charging break most often?

The dust plan should start with the job itself (fewer cuts, a less powerful tool, a different
method), then water or extraction at the tool, and end with the respirator, which HSE calls "the last
line of protection".[3] Indoors, HSE's point is that "the more enclosed a space, the more the dust will
build up", so the job moves outside if it can.[3] The battery answer is yours. LFB's own example of the
habit to drop is "it's best not to leave your phone plugged in overnight".[17]
:::

## For your plan

To the **stop list**: any material you think might be asbestos or old paint before you cut, sand or
scrape it; a swollen battery; any old tin of methylene chloride stripper; any tool with a cracked
handle, sprung jaws, a mushroomed head or a damaged lead. To the **calendar**: a look over the kit and
the chargers, each spring and autumn, with the other seasonal checks. And for the **repair write-up**:
if your one repair makes dust, step 2 of the exercise is its plan.

## Connections

Lesson 5, "Electricity: what trips, and why", explained what an RCD does, and lesson 6, "Plugs, leads
and electrical fires", gave the rule that a plug is opened only when it's out of the socket. Power
tools have the same rule for blades and bits. Lesson 11, "Carbon monoxide and the alarms that
warn you", is about carbon monoxide from burning; methylene chloride is a route that has nothing to do
with a flue. First Aid and CPR lesson 10, "Burns, cuts and small injuries", has what to do about
something in the eye.

Drilling into a wall, with the cables, pipes and materials that can be hidden in it, has a lesson of
its own, lesson 14, "Before you drill: cables, pipes and what's in the wall", which uses this lesson's dust and
masks. Lesson 15, "Ladders and working at height", takes the same tools up a ladder, where the risk is
the fall. Lesson 19, "Asbestos, lead paint and radon: what not to disturb", teaches the materials that
stop a cut or a sanding job before it starts. And Tools and Making Things, later on the Core, takes the
kit further than repairs.

## Go deeper

- **HSE, [*Construction dust*, CIS36](https://www.hse.gov.uk/pubns/cis36.pdf).** Six pages, free, read
  here in full. Written for employers, but the clearest account this course found of the three kinds of
  dust and the order of the controls, with a table of common jobs.
- **OSHA, [*Hand and Power Tools*, OSHA 3080](https://www.osha.gov/sites/default/files/publications/osha3080.pdf).**
  Read here in full. Dated, workplace-facing, and still the plainest list of how tools hurt people and
  what stops it.
- **EPA, ["Volatile Organic Compounds' Impact on Indoor Air Quality"](https://www.epa.gov/indoor-air-quality-iaq/volatile-organic-compounds-impact-indoor-air-quality).**
  Read here in full. The US government's page on solvents and paints at home, with its ventilation
  advice.
- **London Fire Brigade, ["Batteries and chargers"](https://www.london-fire.gov.uk/safety/the-home/electrical-items/batteries-and-chargers/).**
  Read here in full. Short, and the charging advice this lesson used.

## Sources

1. Occupational Safety and Health Administration, [*Hand and Power Tools*, OSHA 3080](https://www.osha.gov/sites/default/files/publications/osha3080.pdf), 2002 (Revised). **Read: the whole booklet; general precautions, guards, electric tools and abrasive wheels read closely; re-read 25 September 2026.** US workplace guidance, written for employers and employees. <!-- SOURCES Part B §1; lesson 11 gate closure -->
2. Health and Safety Executive, ["All you need to know about disposable dust masks"](https://www.hse.gov.uk/agriculture/dustmasks.htm). **Read: full text, re-read 25 September 2026.** No date shown; written for agriculture, and it points to guidance due in autumn 2005. <!-- SOURCES Part B §1; lesson 11 gate closure -->
3. Health and Safety Executive, [*Construction dust*, Construction Information Sheet No 36 (Revision 3)](https://www.hse.gov.uk/pubns/cis36.pdf), first published 06/13, revised 03/20. **Read: full text, 26 September 2026.** Written for employers. <!-- lesson 11 gate closure -->
4. Health and Safety Executive, [*Wood dust: Controlling the risks*, Information Sheet No 23 (Revision 3)](https://www.hse.gov.uk/pubns/wis23.pdf), first published 11/12, revised 06/22. **Read: pages 1 to 3, 6 and 7 (the risks, the law, respiratory protection), 26 September 2026.** Written for woodworking employers. <!-- lesson 11 gate closure -->
5. Electrical Safety First, ["Advice for DIY-ers"](https://www.electricalsafetyfirst.org.uk/guidance/advice-for-you/diyers/). **Read: full text, re-read 25 September 2026.** A UK charity that campaigns on electrical safety; the survey's date isn't given. <!-- SOURCES Part D D34; lesson 11 gate closure -->
6. Health and Safety Executive, ["Cancer and construction: Silica"](https://www.hse.gov.uk/construction/healthrisks/cancer-and-construction/silica-dust.htm) and ["Silicosis: causes and risk controls"](https://www.hse.gov.uk/lung-disease/silicosis.htm). **Read: full text of both, re-read 25 September 2026.** <!-- SOURCES Part D D43; lesson 11 gate closure -->
7. Occupational Safety and Health Administration, ["Silica, Crystalline"](https://www.osha.gov/silica-crystalline) (overview). **Read: the overview page, 26 September 2026.** <!-- SOURCES Part D D44; lesson 11 gate closure -->
8. [29 CFR 1926.1153, Respirable crystalline silica](https://www.osha.gov/laws-regs/regulations/standardnumber/1926/1926.1153) (construction), on OSHA's site. **Read: paragraphs (a) to (f), including Table 1 and the housekeeping rules, 26 September 2026.** Federal; binds employers. <!-- lesson 11 gate closure -->
9. National Institute for Occupational Safety and Health, "Find Filtering Facepiece Respirators on the Certified Equipment List", 18 February 2025, Internet Archive capture 20260923185140 of [cdc.gov](https://www.cdc.gov/niosh/ppe/niosh-approved-respirators/ffr-cel.html) (the live page blocks automated readers). **Read: full page.** <!-- SOURCES Part B §1; lesson 11 gate closure -->
10. [29 CFR 1910.134, Respiratory protection](https://www.osha.gov/laws-regs/regulations/standardnumber/1910/1910.134), on OSHA's site. **Read: the definitions, paragraph (d)(3)(i)(A) with Table 1 and its notes, and paragraph (g)(1), 26 September 2026.** Federal; binds employers. <!-- lesson 11 gate closure G-H4 -->
11. US Environmental Protection Agency, ["Volatile Organic Compounds' Impact on Indoor Air Quality"](https://www.epa.gov/indoor-air-quality-iaq/volatile-organic-compounds-impact-indoor-air-quality), last updated 23 June 2026. **Read: full text, re-read 25 September 2026.** <!-- SOURCES Part B §4, Part D D45; lesson 11 gate closure -->
12. US Environmental Protection Agency, ["Risk Management for Methylene Chloride"](https://www.epa.gov/assessing-and-managing-chemicals-under-tsca/risk-management-methylene-chloride), last updated 6 March 2026. **Read: full text, 25 September 2026.** EPA's summary of its 2019 and 2024 rules; the rules' own text was not read. <!-- lesson 11 gate closure G-R3 -->
13. US Consumer Product Safety Commission, [Publication 5133, "Preventing Home Fires: Arc Fault Circuit Interrupters (AFCIs)"](https://www.cpsc.gov/s3fs-public/5133.pdf), dated R042012. **Read: full text.** <!-- SOURCES Part A §3 -->
14. Health and Safety Executive, [*Electricity at work: Safe working practices*, HSG85](https://www.hse.gov.uk/pubns/priced/hsg85.pdf), third edition, 2013. **Read: the whole PDF extracted; "Proving dead" paragraphs 53 to 55 read.** Workplace guidance, used here only for what it says about non-contact testers. <!-- SOURCES Part D D36 -->
15. North Seattle College, continuing education, "Basic Hands-On Home Repair and Maintenance". **Read: full course page.** <!-- SOURCES Part A §7 -->
16. London Fire Brigade, ["E-bikes and e-scooters: fire safety advice"](https://www.london-fire.gov.uk/safety/lithium-batteries/). **Read: full text, re-read 25 September 2026.** No date shown; London only. <!-- SOURCES Part B §1; lesson 11 gate closure -->
17. London Fire Brigade, ["Batteries and chargers"](https://www.london-fire.gov.uk/safety/the-home/electrical-items/batteries-and-chargers/). **Read: full text, re-read 25 September 2026.** No date shown; written about phones, laptops and similar devices. <!-- SOURCES Part B §1; lesson 11 gate closure -->
