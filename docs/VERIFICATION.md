...
| Whether the TTS daily quota is actually out | `npm run state` | Reads `audio-out/work/**/manifest.json`. A 429 carries a `retryDelay` and the script used to trust it alone, so **one refusal told every session "quota out, work content instead" for the whole delay it named**. The quota is a rolling window and that delay is Google's estimate of when the oldest request ages out, so it over-predicts whenever anything else expires first. On 2026-09-19 it was still printing 11.9 hours remaining while a render went through. **A success later than the last refusal now clears the block**, because a render that completed is proof the window has room |
| Every Mental Fitness lesson carries the crisis callout, word for word | `npm run validate` | Fails on published, warns on drafts. The standard text lives once in `scripts/build.mjs`; a lesson that drops the box or keeps an old number fails. Added 2026-09-23 with the course's first lesson, under its outline's decision 1 |
| Every Nutrition lesson carries the safety callout (education not advice, who to ask first, Beat and ANAD), word for word | `npm run validate` | Fails on published, warns on drafts. Same mechanism as Mental Fitness's; the text lives once in `scripts/build.mjs`, with a test in `scripts/tests/build.test.mjs`. Added 2026-09-24 under the Nutrition outline's decision 1 |
| Whether Google's prepaid credit balance is out (HTTP 402) | `npm run state` | Reads the same manifests. A 402 has no `retryDelay` because it lasts until John tops up, so it blocks the render branch **until a later render succeeds**, prints one line saying so, and moves the next action on to content. Refused before any audio is generated, so it costs nothing. The monthly cap and the prepaid balance are different numbers and either can run out first; this file still cannot see the balance itself, only the last refusal. Added 2026-09-23, after `npm run state` named a render as the one thing to do for four days while every attempt would have been refused |
...
| An explanation naming a distractor by its position ("the second answer", "the last option") | `npm run quiz` | Advisory, and it prints the sentence so every hit is read by eye, because an ordinal can legitimately point at something enumerated in the stem. `/draft-lesson` defect 17. It matters more than it reads: the day somebody permutes a quiz's options to fix the clustering defect, a positional explanation starts describing the wrong option and nothing fails. Three courses were permuted on 2026-09-19 and two Focus explanations survived it by luck alone. Added the same day; **it reports 34 items across seven courses on its first run, and they are real.** They are written up in `docs/QUEUE.md` rather than fixed, under root rule 9 |
| A quotation in a lesson that its own `SOURCES.md` does not contain | `npm run quotes [course]` | **A reviewer's tool, deliberately not a build check.** Memory lesson 5 printed a passage in quotation marks, attributed to the authors and carrying a source marker, that nobody wrote: it was the research file's own unquoted commentary turned into a quotation. Every other quotation in that lesson matched character for character, which is why reading for sense would not catch it. It is the one class of error the institute's method exists to make impossible, and it is mechanical to find. It is not a build gate because a repo-wide run reports 148 misses in 458 quotations and nearly all are legitimate: Bible Basics quotes scripture, Logic and Argument quotes the arguments it analyses, Reading Well quotes the passages it teaches on, and none of those belong in `SOURCES.md`. A check that cried wolf on a third of the repo would be ignored within a week. Run it on any course whose sources are papers. **Widened the same day**, after a Stage 4 review found that the one quotation a whole course rested on was invisible to it: the marker followed a closing bold span, and a second real quotation fell under the 40-character floor. It now strips bold before matching and takes quotations from 25 characters, which found eleven more across the repo. **Widened again on 2026-09-19**: a quotation long enough to need a block quote wraps, and every continuation line then begins "> ", which the normaliser was keeping inside the quoted string. Three verbatim quotations of two real documents in Sleep lesson 3 read as misses because of it. The marker is stripped from the start of every line before matching. **On 2026-09-23 the same fix was found to cover only the lesson side**: a wrapped block quote in `SOURCES.md` still carried its markers, and two verbatim quotations of an announcement read as misses. The normaliser now strips them on both sides On its first run it found one live defect in Focus and Deep Work lesson 6, a quoted sentence about what training does produce that the research file does not hold; that lesson now paraphrases the abstract sentence it did read and says so |
| An unbalanced `**` run, or four asterisks in a row, in a paragraph of a lesson | `npm run validate` | Fails on a published course, warns on a draft. It renders as literal asterisks on the page and the emphasis it was meant to carry disappears, and nothing caught it before: two Stage 4 reviewers found two of them by eye on 2026-09-19, which is the wrong way to find a mechanical defect. A repo-wide scan the same day found **fourteen across five courses, three of them live**, and every one came from an edit rather than from drafting: a scripted bold-density cut that took one span out of a pair, or a span wrapped that was already wrapped. All fourteen are fixed and the check is quiet across the repo |
| Every link in a lesson sitting in Go deeper or Sources, with none in the teaching prose | `npm run validate` | Warns. 4.5 asks for links where the reader meets the source, and a paper quoted on page one should be reachable from page one. **Five consecutive Note-Taking lessons put every link below the teaching prose**, five consecutive Stage 4 reviews raised it, each was fixed in place, and the sixth draft did it again, which is root rule 10's trigger. The existing check only asked whether a lesson linked anything anywhere. On its first repo-wide run it found **35 lessons across eight courses**, recorded in `docs/QUEUE.md` rather than fixed |
| The same 25-word passage appearing twice in one lesson's teaching prose | `npm run validate` | Warns. **Two distinct routes have produced one, both during a fix pass and both invisible in the diff**: a passage promoted out of a `:::checkpoint` into a section of its own and never deleted from the checkpoint (Note-Taking lesson 5, found by lesson 7's reviewer), and a scripted edit that inserted where it should have replaced. Block quotes and anything inside quotation marks are stripped first, because quoted source material is repeated on purpose, and emphasis is stripped too, since a copy that gained or lost a bold span is still a copy. Quiet across the repo on its first run |
