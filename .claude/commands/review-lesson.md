...
**Then, in the same fix pass, run the voice checks** (this replaced the separate `/voice-pass` stage on 2026-09-17; `docs/DECISIONS.md` §13): search the file for em dashes and every word on the style guide's banned list and rewrite each sentence; remove bold from anything that is not a key term; count the think-blocks and add, from content already in the lesson, until there are two or three `:::predict` before results, a `:::checkpoint` after each hard idea, and one worked example with a gap the reader fills before the next step is shown; check every `[n]` marker has a footnote; measure `minutes:` with `npm run minutes`; read the opening aloud. Then run `npm run validate` **and check its exit code, not its output** (a wrapper that grepped for "ok" passed an unclosed block on 2026-09-17), and open the lesson in the browser if it has a chart in it.
...
Escalate to Tier B and review again only on the triggers above. Otherwise the lesson is done, and it needs its podcast episode: `curriculum/audio-debt.yaml` is what stops the course being called finished without one.
