...
rendered voices are fixed in `scripts/podcast.mjs` (John is the Carter preset, Haley is
Alice; John chose them by ear). One episode per session.
...
```
node scripts/podcast.mjs render $ARGUMENTS          # dry run: confirm cost (~$0.30) and shape
...
Rendering can take up to 20 minutes on a cold start. If the render fails, check the fal
