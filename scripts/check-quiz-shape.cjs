  ...
  for (const s of dirs("courses"))
    for (const c of dirs(`courses/${s}`))
      for (const d of ["lessons", "assessments"]) {
        ...
        if (fs.existsSync(p)) for (const f of fs.readdirSync(p)) if (f.endsWith(".md")) out.push(`${p}/${f}`);
      }
  return out;
}
...
for (const f of files) {
  ...
  for (let k = 0; k < 4; k++) strategies["index " + L[k]] = score(() => k);
  ...
  const unused = [...L].slice(0, Math.min(4, Math.max(...q.map(it => it.options.length)))).filter(c => !indices.includes(c));
